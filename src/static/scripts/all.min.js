...
(function(){var q=50;var r=1.5;var u=10;var c=1.2;var o=1.2;var j=0.5;var F="diabetes",e="LLDA",g,k,a;var f=0,v=0,E=0,d=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(J){B("#startdatepicker","#startDatepickerBtn",true);w(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(J){B("#enddatepicker","#endDatepickerBtn",false);w(true,"#enddatepicker","endDateText");s();}});$("#startdatepicker").datepicker("setDate","01/01/2001");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();x();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){G("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(J){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(J){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(J){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(J){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function G(J){return document.getElementById(J);}function x(){var K=document.URL.split("/");var J=e=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";g=K[0]+"//"+K[2]+"/"+J+"?keywords="+F;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);g+="&start_date="+startDate+"&end_date="+endDate;g+="&limit="+q;if(e!="DUAL"){g+="&model="+e;}var L=new XMLHttpRequest();L.open("GET",g,true);L.onreadystatechange=z(L);L.send(null);n();b();}function z(J){return function(){if(J.readyState==4){if(J.status==200){i();h(J.responseText);}else{console.log("Retrying..");k=setTimeout(function(){console.log(g);var K=new XMLHttpRequest();K.open("GET",g,true);K.onreadystatechange=z(K);K.send(null);},60000);}}};}function h(J){try{a=JSON.parse(J);}catch(K){a=J;}if(a==""){G("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){G("topics").innerHTML="NO TOPIC FOUND!";return;}y();m();E=0;setTimeout(I,3000);}function D(J){G("topics").innerHTML=J||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function i(){$("#topics").empty();}function y(J){l.draw(J);}function s(){if(k!=null){clearTimeout(k);}if(l!=null){l.remove();}}function I(K,J){try{if(E){return;}if(!K){K=$("circle");}if(typeof J=="undefined"){J=0;}if(K[J]){d=$(K[J]).mouseover();}}catch(L){}}function n(){if(d){d.mouseout();E=1;d=null;}}function m(J){var J="Topics extracted from medical abstracts published";J+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(F=="diabetes"){J+=" about Diabetes";}else{J+=" about Cancer";}$("#topics").prepend("<span>"+J+"</span>");}function H(K,J){if(F!=J){F=J;s();A(K);}return false;}function t(K,J){if(e!=J){e=J;s();p(K);}return false;}function w(K,J,L){if(K){G(L).innerHTML=jQuery(J).datepicker("getDate").toDateString().substring(4);}else{G(L).innerHTML="pick a date";}}function A(J){$("#regions a").removeClass("current");$(J).addClass("current");}function p(J){$("#models a").removeClass("current");$(J).addClass("current");}function B(J,K,L){if((L&&f)||(!L&&v)){$(J).slideUp();$(K).removeClass("datepicker-open");}else{$(J).slideDown();$(K).addClass("datepicker-open");}if(L){f=!f;}else{v=!v;}return false;}var C={bubbleChart:function(){var L=G("topics"),K,M,J;return{draw:function(){L.innerHTML=null;var Y=L.offsetWidth;var ah=L.offsetHeight;J=d3.select("#topics").append("svg").attr("width",Y).attr("height",ah);var O=a.links;var V=d3.forceSimulation().force("link",d3.forceLink().id(function(am){return am.uid;}).distance(function(am){if(e=="LDA"){console.log(am.value*c);return(1/(am.value*c))*20000;}else{if(e=="LLDA"){console.log("weird");console.log(am.value*j);return(1/(am.value*j))*20000;}else{return(1/(am.value*o))*20000;}}})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(Y/2,ah/2));var W=J.append("g").attr("class","links").selectAll("line").data(O).enter().append("line");M={};O.forEach(function(am){M[am.source+","+am.target]=1;});console.log(M);var ae=Object.keys(a.topics).map(function(am){return a.topics[am];});var Q=ae.length;var ai=ae[0].score;for(var aa=0;aa<Q;aa++){if(ae[aa].score>ai){ai=ae[aa].score;}}var ag=d3.scaleOrdinal().range(["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);var P=d3.scaleOrdinal().range(["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]);K=J.selectAll(".node").data(ae).enter().append("circle").attr("class","node").attr("cx",function(am){return am.x;}).attr("cy",function(am){return am.y;}).attr("r",0).style("fill",function(am){return X(am);}).style("stroke",function(an,am){return d3.rgb(an.color).darker(2);}).call(d3.drag().on("start",ab).on("drag",Z).on("end",ad));V.nodes(ae).on("tick",U);V.force("link").links(O);function U(){W.attr("x1",function(am){return am.source.x;}).attr("y1",function(am){return am.source.y;}).attr("x2",function(am){return am.target.x;}).attr("y2",function(am){return am.target.y;});K.attr("cx",function(am){return am.x;}).attr("cy",function(am){return am.y;});}K.transition().duration(1000).attr("r",function(am){if(e=="LLDA"){return am.score*500*u;}else{if(e=="DUAL"&&am.uid<0){return am.score*500*(u*0.8);}}return am.score*500*r;});J.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){n();var am=d3.select(this);am.moveToFront();var an=this.__data__;return'<div class="tipsy-topic">'+N(an.nameTokens)+'</div><span class="tipsy-time">'+R((an.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var an=this.__data__;if(e!="DUAL"){ac(an.docs,an.wordsProb,an.nameTokens);if(an.years.length>0){af(an.years,an.maxYearCount,"tempHist");}else{var am=G("tempHist");am.innerHTML=null;$("#tempHist").prepend("<span>Not enough data</span>");}if(an.forecastYears.length>0){af(an.forecastYears,an.maxYearCount,"forTempHist");}else{var am=G("forTempHist");am.innerHTML=null;$("#forTempHist").prepend("<span>Not enough data to forecast</span>");}}S(an,0.1);});function ak(an,am){return M[an.uid+","+am.uid]||M[am.uid+","+an.uid];}function S(an,am){J.selectAll("circle, line").style("opacity",am);W.style("opacity",function(ao){return ao.source===an||ao.target===an?1:am;});K.style("opacity",function(ao){return an.uid==ao.uid||ak(an,ao)?1:am;});}function aj(ao){var am="";for(var an=0;an<ao.length;an++){am+=ao[an]+" ";}titleLength=am.length<20?doc.title.length:20;return am.slice(0,titleLength)+"...";}function al(ar,aq){var ao='<div id="mod'+ar+'" class="modal"><div class="modal-content"><span class="close" id=clo'+ar+">&times;</span><p><b>"+aq.title+"</b></p>";var at=aq.uiText.split(" ");ao+="<p>";for(var am=0;am<aq.topicList.length;am++){var an=aq.topicList[am];ao+='<a class="modalReg" id="top'+an+'">';ao+=aj(a.topics[an].nameTokens)+"</a>";topId="#top"+an;$("#docs").on({mouseenter:function(av){$(".snip").css({opacity:0.2});var au=".sniptop"+(this.id).slice(3,(this.id).length);$(au).css({opacity:1});},mouseleave:function(au){$(".snip").css({opacity:1});}},topId);}ao+="</p><p>";for(var am=0;am<at.length;am++){ao+='<span class="snip ';for(var ap=0;ap<aq.topicList.length;ap++){var an=aq.topicList[ap];if(am>=aq.summaries[an][0]&&am<=aq.summaries[an][1]){ao+=" sniptop"+an;}}ao+='">'+at[am]+" </span>";}ao+="</p></div></div>";return ao;}function T(at,ar){var ap='<div id="mod'+at+'" class="modal"><div class="modal-content"><span class="close" id=clo'+at+">&times;</span><p><b>"+ar.title+"</b></p>";var an={};var au=ar.uiText.split(" ");for(var am=0;am<au.length;am++){an[au[am].replace(/\W/g,"").toLowerCase()]={topicId:"",prob:0};}ap+="<p>";for(var am=0;am<ar.topicList.length;am++){var ao=ar.topicList[am];ap+='<a class="region" id="top'+ao+'">';ap+=aj(a.topics[ao].nameTokens)+"</a>";topId="#top"+ao;$("#docs").on({mouseenter:function(aw){$(this).attr("class","current");$(".word").css({opacity:0});var av=".wtop"+(this.id).slice(3,(this.id).length);$(av).css({opacity:1});$(".wtop").css({opacity:0.3});},mouseleave:function(av){$(this).attr("class","region");$(".word").css({opacity:1});}},topId);for(var aq=0;aq<a.topics[ao].wordsProb.length;aq++){word=a.topics[ao].wordsProb[aq][0];prob=a.topics[ao].wordsProb[aq][1];if((word in an)&&a.topics[ao].score*prob>an[word].prob){an[word].topicId=ao;an[word].prob=a.topics[ao].score*prob;}}}ap+="</p><p>";for(var am=0;am<au.length;am++){ap+='<span class="word wtop';key=au[am].replace(/\W/g,"").toLowerCase();if(key in an){ap+=an[key].topicId;}ap+='">'+au[am]+" </span>";}ap+="</p></div></div>";return ap;}function ac(at,ao,ar){var ap=G("docs");ap.innerHTML=null;listLength=at.length;for(var am=0;am<listLength&&am<5;am++){pmid=at[am];doc=a.docs[pmid];titleLength=doc.title.length<21?doc.title.length:21;title=doc.title.slice(0,titleLength)+"...";ap.innerHTML+='<button class="region" id="but'+pmid+'">'+title+"</button>";ap.innerHTML+=al(pmid,doc);var aq="#but"+pmid;$("#docs").on("click",aq,function(au){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="block";});var an="#clo"+pmid;$("#docs").on("click",an,function(au){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function af(av,ap,aA){var ar=new Array();var ao=new Date(2050,1,1);var au=new Date(1960,1,1);for(var az=0;az<av.length;az++){date=new Date(av[az],1,1);if(date<ao){ao=date;}if(date>au){au=date;}ar.push(date);}var an=G(aA);an.innerHTML=null;J=d3.select("#"+aA).append("svg").attr("width",an.offsetWidth*2).attr("height",an.offsetHeight/3);var aq={top:10,right:30,bottom:30,left:40};var am=J.attr("width")-aq.left-aq.right;var aB=J.attr("height")-aq.top-aq.bottom;var at=J.append("g").attr("transform","translate("+aq.left+","+aq.top+")");minEndDate=new Date(ao.getFullYear()+5,1,1);au=minEndDate.getTime()<au.getTime()?au:minEndDate;var ay=d3.scaleTime().domain([ao,au]).rangeRound([0,am]);var aC=d3.histogram().domain(ay.domain()).thresholds(ay.ticks(d3.timeYear))(ar);var ax=d3.scaleLinear().domain([0,ap]).range([aB,0]);var aw=at.selectAll(".bar").data(aC).enter().append("g").attr("class","bar").attr("transform",function(aD){return"translate("+ay(aD.x0)+","+ax(aD.length)+")";});aw.append("rect").attr("x",1).attr("width",ay(aC[0].x1)-ay(aC[0].x0)-1).attr("height",function(aD){return aB-ax(aD.length);});if(ar.length>0){at.append("g").attr("class","axisBlue").attr("transform","translate(0,"+aB+")").call(d3.axisBottom(ay));at.append("g").attr("class","axisBlue").call(d3.axisLeft(ax));}title="";if(aA=="tempHist"){title="Temporal trend for topic";}$("#"+aA).prepend("<span>"+title+"</span>");}function ab(am){if(!d3.event.active){V.alphaTarget(0.3).restart();}am.fx=am.x;am.fy=am.y;}function Z(am){am.fx=d3.event.x;am.fy=d3.event.y;}function ad(am){if(!d3.event.active){V.alphaTarget(0);}am.fx=null;am.fy=null;}function X(am){if(am.uid>=0){am.color=ag(Math.floor(am.score/(ai/5)));}else{am.color=P(Math.floor(am.score/(ai/5)));}return am.color;}function R(am){if(am==0){return"";}else{if(am>59){return Math.floor(am/60)+" h. "+R(am%60);}else{return am+" avg score.";}}}function N(ao){var ap=ao.length;var an="";for(var am=0;am<ap;am++){an+=ao[am]+" ";}return an;}},remove:function(){if(K!=null){K.transition().duration(1000).attr("r",0).remove();}if(J!=null){J.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();}var N=G("docs");N.innerHTML=null;var N=G("tempHist");N.innerHTML=null;var N=G("forTempHist");N.innerHTML=null;x();}};}};var l=C.bubbleChart();jQuery("#cancerBtn").click(function(){H(this,"cancer");});jQuery("#diabetesBtn").click(function(){H(this,"diabetes");});jQuery("#ldaBtn").click(function(){t(this,"LDA");});jQuery("#lldaBtn").click(function(){t(this,"LLDA");});jQuery("#dualBtn").click(function(){t(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){B("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){B("#enddatepicker","#endDatepickerBtn",false);});})();
