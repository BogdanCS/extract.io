...
(function(){var l=50;var x="diabetes",d="LDA",a;var e=0,o=0,w=0,c=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#startdatepicker","#startDatepickerBtn",true);p(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#enddatepicker","#endDatepickerBtn",false);p(true,"#enddatepicker","endDateText");m();}});$("#startdatepicker").datepicker("setDate","01/01/2012");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();q();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){y("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(B){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(B){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function y(B){return document.getElementById(B);}function q(){var D=document.URL.split("/");var B=d=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";var C=D[0]+"//"+D[2]+"/"+B+"?keywords="+x;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);C+="&start_date="+startDate+"&end_date="+endDate;C+="&limit="+l;if(d!="DUAL"){C+="&model="+d;}var E=new XMLHttpRequest();E.open("GET",C,true);E.onreadystatechange=function(){if(E.readyState==4){g();if(E.status==200){f(E.responseText);}else{v();}}};E.send(null);j();b();}function f(B){try{a=JSON.parse(B);}catch(C){a=B;}if(a==""){y("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){y("topics").innerHTML="NO TOPIC FOUND!";return;}r();i();w=0;setTimeout(A,3000);}function v(B){y("topics").innerHTML=B||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function g(){$("#topics").empty();}function r(B){h.draw(B);}function m(){h.remove();}function A(C,B){try{if(w){return;}if(!C){C=$("circle");}if(typeof B=="undefined"){B=0;}if(C[B]){c=$(C[B]).mouseover();}}catch(D){}}function j(){if(c){c.mouseout();w=1;c=null;}}function i(B){var B="Topics extracted from medical abstracts published";B+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(x=="diabetes"){B+=" about Diabetes";}else{B+=" about Cancer";}$("#topics").prepend("<span>"+B+"</span>");}function z(C,B){if(x!=B){x=B;m();s(C);}return false;}function n(C,B){if(d!=B){d=B;m();k(C);}return false;}function p(C,B,D){if(C){y(D).innerHTML=jQuery(B).datepicker("getDate").toDateString().substring(4);}else{y(D).innerHTML="pick a date";}}function s(B){$("#regions a").removeClass("current");$(B).addClass("current");}function k(B){$("#models a").removeClass("current");$(B).addClass("current");}function t(B,C,D){if((D&&e)||(!D&&o)){$(B).slideUp();$(C).removeClass("datepicker-open");}else{$(B).slideDown();$(C).addClass("datepicker-open");}if(D){e=!e;}else{o=!o;}return false;}var u={bubbleChart:function(){var D=y("topics"),C,E,B;return{draw:function(){D.innerHTML=null;var P=D.offsetWidth;var Y=D.offsetHeight;B=d3.select("#topics").append("svg").attr("width",P).attr("height",Y);var G=a.links;var M=d3.forceSimulation().force("link",d3.forceLink().id(function(ab){return ab.uid;}).distance(function(ab){return(1/ab.value)*20000;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(P/2,Y/2));var N=B.append("g").attr("class","links").selectAll("line").data(G).enter().append("line");E={};G.forEach(function(ab){E[ab.source.uid+","+ab.target.uid]=1;});var V=a.topics;var H=V.length;var Z=V[0].score;for(var R=0;R<H;R++){if(V[R].score>Z){Z=V[R].score;}}var X=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);C=B.selectAll(".node").data(V).enter().append("circle").attr("class","node").attr("cx",function(ab){return ab.x;}).attr("cy",function(ab){return ab.y;}).attr("r",0).style("fill",function(ab){return O(ab);}).style("stroke",function(ac,ab){return d3.rgb(ac.color).darker(2);}).call(d3.drag().on("start",S).on("drag",Q).on("end",U));M.nodes(V).on("tick",L);M.force("link").links(G);function L(){N.attr("x1",function(ab){return ab.source.x;}).attr("y1",function(ab){return ab.source.y;}).attr("x2",function(ab){return ab.target.x;}).attr("y2",function(ab){return ab.target.y;});C.attr("cx",function(ab){return ab.x;}).attr("cy",function(ab){return ab.y;});}C.transition().duration(1000).attr("r",function(ab){return ab.score*500;});B.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){j();var ab=d3.select(this);ab.moveToFront();var ac=this.__data__;return'<div class="tipsy-topic">'+F(ac.nameTokens)+'</div><span class="tipsy-time">'+I((ac.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var ab=this.__data__;T(ab.docs);W(ab.years);J(ab,0.1);});function aa(ac,ab){return E[ac.uid+","+ab.uid]||E[ab.uid+","+ac.uid];}function J(ac,ab){B.selectAll("circle, line").style("opacity",ab);N.style("opacity",function(ad){return ad.source===ac||ad.target===ac?1:ab;});C.style("opacity",function(ad){return aa(ac,ad)?1:ab;});}function K(ac,ab){modal='<div id="mod'+ac+'" class="modal"><div class="modal-content"><span class="close" id=clo'+ac+">&times;</span><p>"+ab+"</p></div>";"</div>";return modal;}function T(af){var ad=y("docs");ad.innerHTML=null;listLength=af.length;for(var ab=0;ab<listLength&&ab<5;ab++){lines=af[ab].split("<br>");pmid=(lines[0].split(":"))[1];console.log(pmid);titleLength=lines[1].length<21?lines[1].length:21;title=lines[1].slice(0,titleLength)+"...";ad.innerHTML+='<button id="but'+pmid+'">'+title+"</button>";ad.innerHTML+=K(pmid,af[ab]);var ae="#but"+pmid;$("#docs").on("click",ae,function(ag){id="mod"+(this.id).slice(3,(this.id).length);console.log(id);document.getElementById(id).style.display="block";});var ac="#clo"+pmid;$("#docs").on("click",ac,function(ag){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function W(ag){var ae=new Array();for(var ak=0;ak<ag.length;ak++){ae.push(new Date(ag[ak],1,1));}var ab=y("tempHist");ab.innerHTML=null;B=d3.select("#tempHist").append("svg").attr("width",ab.offsetWidth*2).attr("height",ab.offsetHeight/3);var ad={top:10,right:30,bottom:30,left:30};var ac=B.attr("width")-ad.left-ad.right;var al=B.attr("height")-ad.top-ad.bottom;var af=B.append("g").attr("transform","translate("+ad.left+","+ad.top+")");var aj=d3.scaleTime().domain([new Date(2006,1,1),new Date(2017,1,1)]).rangeRound([0,ac]);var am=d3.histogram().domain(aj.domain()).thresholds(aj.ticks(d3.timeYear))(ae);var ai=d3.scaleLinear().domain([0,d3.max(am,function(an){return an.length;})]).range([al,0]);var ah=af.selectAll(".bar").data(am).enter().append("g").attr("class","bar").attr("transform",function(an){return"translate("+aj(an.x0)+","+ai(an.length)+")";});ah.append("rect").attr("x",1).attr("width",aj(am[0].x1)-aj(am[0].x0)-1).attr("height",function(an){return al-ai(an.length);});if(ae.length>0){af.append("g").attr("class","axisBlue").attr("transform","translate(0,"+al+")").call(d3.axisBottom(aj));af.append("g").attr("class","axisBlue").call(d3.axisLeft(ai));}$("#tempHist").prepend("<span>Temporal trend for the topic</span>");}function S(ab){if(!d3.event.active){M.alphaTarget(0.3).restart();}ab.fx=ab.x;ab.fy=ab.y;}function Q(ab){ab.fx=d3.event.x;ab.fy=d3.event.y;}function U(ab){if(!d3.event.active){M.alphaTarget(0);}ab.fx=null;ab.fy=null;}function O(ab){ab.color=X(Math.floor(ab.score/(Z/5)));return ab.color;}function I(ab){if(ab==0){return"";}else{if(ab>59){return Math.floor(ab/60)+" h. "+I(ab%60);}else{return ab+" avg score.";}}}function F(ad){var ae=ad.length;var ac="";for(var ab=0;ab<ae;ab++){ac+=ad[ab]+" ";}return ac;}},remove:function(){C.transition().duration(1000).attr("r",0).remove();B.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();q();}};}};var h=u.bubbleChart();jQuery("#cancerBtn").click(function(){z(this,"cancer");});jQuery("#diabetesBtn").click(function(){z(this,"diabetes");});jQuery("#ldaBtn").click(function(){n(this,"LDA");});jQuery("#lldaBtn").click(function(){n(this,"LLDA");});jQuery("#dualBtn").click(function(){n(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){t("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){t("#enddatepicker","#endDatepickerBtn",false);});})();
