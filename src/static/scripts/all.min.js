...
(function(){var s=0,o=1,p=2,f=3,n=50;var q=s,B="diabetes",a;var m=0,A=0,e=null;window.onload=function(){$("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(757382400),maxDate:new Date(),onSelect:function(F){d(this,f);}});$("#datepicker").datepicker("setDate",new Date());b();u();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){C("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(F){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(F){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function C(F){return document.getElementById(F);}function u(){var G=document.URL.split("/");var F=G[0]+"//"+G[2]+"/rpcNewSearch?keywords="+B;if(q==f){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;F+="&start_date="+startDate+"&end_date="+endDate;}F+="&limit="+n;var H=new XMLHttpRequest();H.open("GET",F,true);H.onreadystatechange=function(){if(H.readyState==4){h();if(H.status==200){g(H.responseText);}else{z();}}};H.send(null);k();b();}function g(F){try{a=JSON.parse(F);}catch(G){a=F;}if(a==""){C("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){C("topics").innerHTML="NO TOPIC FOUND!";return;}v();j();A=0;setTimeout(E,3000);}function z(F){C("topics").innerHTML=F||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function h(){$("#topics").empty();}function v(F){i.draw(F);}function r(F){i.remove(F);}function E(G,F){try{if(A){return;}if(!G){G=$("circle");}if(typeof F=="undefined"){F=0;}if(G[F]){e=$(G[F]).mouseover();setTimeout(function(){$(G[F]).mouseout();e=null;E(G,F+1);},5000);}}catch(H){}}function k(){if(e){e.mouseout();A=1;e=null;}}function j(F){var F="Topics extracted from medical abstracts published";if(q==s){F+=" within last 24 hours";}else{if(q==o){F+=" within last week";}else{if(q==p){F+=" within last month";}else{F+=" on "+jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}}}if(B=="diabetes"){F+=" about Diabetes";}else{F+=" about Cancer";}$("#topics").prepend("<span>"+F+"</span>");}function d(G,F){if(q!=F||F==f){q=F;r(u);l(G,F);}return false;}function D(G,F){if(B!=F){B=F;r(u);w(G);}return false;}function t(F){if(F){C("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}else{C("dateText").innerHTML="pick a date";}}function w(F){$("nav a").removeClass("current");$(F).addClass("current");}function l(G,F){if(F==f){t(true);x();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(G){m=true;x();t(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(G).addClass("active");}}}function x(){if(m){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}m=!m;return false;}function c(){if(q==s){return"ld";}else{if(q==o){return"lw";}else{if(q==p){return"lm";}}}}var y={bubbleChart:function(){var H=C("topics"),I,G,F;return{draw:function(){H.innerHTML=null;var V=H.offsetWidth,R=H.offsetHeight;var K=a.topics;var J=K[0].score;var X=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);var L=(3500/V)*(J/50);I=d3.forceSimulation().force("collide",d3.forceCollide(function(Y){return Y.r+8;}).iterations(30)).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(V/2,R/2)).force("y",d3.forceY(0)).force("x",d3.forceX(0));F=d3.select("#topics").append("svg").attr("width",V).attr("height",R);G=F.selectAll(".node").data(K).enter().append("circle").attr("class","node").attr("cx",function(Y){return Y.x;}).attr("cy",function(Y){return Y.y;}).attr("r",0).style("fill",function(Y){return P(Y);}).style("stroke",function(Z,Y){return d3.rgb(Z.color).darker(2);}).call(d3.drag().on("start",S).on("drag",U).on("end",T));var O=function(){G.attr("cx",function(Y){return Y.x;}).attr("cy",function(Y){return Y.y;});};I.nodes(K).on("tick",O);G.transition().duration(1000).attr("r",function(Y){return Y.score/L;});F.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){k();var Y=d3.select(this);Y.moveToFront();var Z=this.__data__;return'<div class="tipsy-topic">'+N(Z.words)+'</div><span class="tipsy-time">'+Q((Z.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var Y=this.__data__;M(Y.docs);W(Y.years);});function M(aa){var Z=C("docs");Z.innerHTML=null;listLength=aa.length;for(var Y=0;Y<listLength&&Y<6;Y++){Z.innerHTML+='<a href="'+aa[Y]+'"> Document #'+Y+"</a>";}}function W(ae){var ab=new Array();for(var ai=0;ai<ae.length;ai++){ab.push(new Date(ae[ai],1,1));}var Y=C("tempHist");Y.innerHTML=null;var ac=d3.select("#tempHist").append("svg").attr("width",Y.offsetWidth*2).attr("height",Y.offsetHeight/3);var aa={top:10,right:30,bottom:30,left:30};var Z=ac.attr("width")-aa.left-aa.right;var aj=ac.attr("height")-aa.top-aa.bottom;var ad=ac.append("g").attr("transform","translate("+aa.left+","+aa.top+")");var ah=d3.scaleTime().domain([new Date(2006,1,1),new Date(2017,1,1)]).rangeRound([0,Z]);var ak=d3.histogram().domain(ah.domain()).thresholds(ah.ticks(d3.timeYear))(ab);var ag=d3.scaleLinear().domain([0,d3.max(ak,function(al){return al.length;})]).range([aj,0]);var af=ad.selectAll(".bar").data(ak).enter().append("g").attr("class","bar").attr("transform",function(al){return"translate("+ah(al.x0)+","+ag(al.length)+")";});af.append("rect").attr("x",1).attr("width",ah(ak[0].x1)-ah(ak[0].x0)-1).attr("height",function(al){return aj-ag(al.length);});if(ab.length>0){ad.append("g").attr("class","axisBlue").attr("transform","translate(0,"+aj+")").call(d3.axisBottom(ah));ad.append("g").attr("class","axisBlue").call(d3.axisLeft(ag));}}function S(Y){if(!d3.event.active){I.alphaTarget(0.3).restart();}Y.fx=Y.x;Y.fy=Y.y;}function U(Y){Y.fx=d3.event.x;Y.fy=d3.event.y;}function T(Y){if(!d3.event.active){I.alphaTarget(0);}Y.fx=null;Y.fy=null;}function P(Y){Y.color=X(Math.floor(Y.score/(J/5)));return Y.color;}function Q(Y){if(Y==0){return"";}else{if(Y>59){return Math.floor(Y/60)+" h. "+Q(Y%60);}else{return Y+" avg prob.";}}}function N(aa){var ab=aa.length;var Z="";for(var Y=0;Y<ab;Y++){Z+=aa[Y]+" ";}return Z;}},remove:function(J){compForce.force("gravity",gravity(0.001)).restart();G.transition().duration(1000).attr("r",0).remove();F.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",J);}};}};var i=y.bubbleChart();jQuery("#cancerBtn").click(function(){D(this,"cancer");});jQuery("#diabetesBtn").click(function(){D(this,"diabetes");});jQuery("#lastDayBtn").click(function(){d(this,s);});jQuery("#lastWeekBtn").click(function(){d(this,o);});jQuery("#lastMonthBtn").click(function(){d(this,p);});jQuery("#datepickerBtn").click(function(){x();});})();
