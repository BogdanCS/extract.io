...
(function(){var l=50;var x="diabetes",d="LLDA",a;var e=0,o=0,w=0,c=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#startdatepicker","#startDatepickerBtn",true);p(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#enddatepicker","#endDatepickerBtn",false);p(true,"#enddatepicker","endDateText");m();}});$("#startdatepicker").datepicker("setDate","01/01/2012");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();q();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){y("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(B){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(B){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function y(B){return document.getElementById(B);}function q(){var D=document.URL.split("/");var B=d=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";var C=D[0]+"//"+D[2]+"/"+B+"?keywords="+x;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);C+="&start_date="+startDate+"&end_date="+endDate;C+="&limit="+l;if(d!="DUAL"){C+="&model="+d;}var E=new XMLHttpRequest();E.open("GET",C,true);E.onreadystatechange=function(){if(E.readyState==4){g();if(E.status==200){f(E.responseText);}else{v();}}};E.send(null);j();b();}function f(B){try{a=JSON.parse(B);}catch(C){a=B;}if(a==""){y("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){y("topics").innerHTML="NO TOPIC FOUND!";return;}r();i();w=0;setTimeout(A,3000);}function v(B){y("topics").innerHTML=B||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function g(){$("#topics").empty();}function r(B){h.draw(B);}function m(){h.remove();}function A(C,B){try{if(w){return;}if(!C){C=$("circle");}if(typeof B=="undefined"){B=0;}if(C[B]){c=$(C[B]).mouseover();setTimeout(function(){$(C[B]).mouseout();c=null;A(C,B+1);},5000);}}catch(D){}}function j(){if(c){c.mouseout();w=1;c=null;}}function i(B){var B="Topics extracted from medical abstracts published";B+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(x=="diabetes"){B+=" about Diabetes";}else{B+=" about Cancer";}$("#topics").prepend("<span>"+B+"</span>");}function z(C,B){if(x!=B){x=B;m();s(C);}return false;}function n(C,B){if(d!=B){d=B;m();k(C);}return false;}function p(C,B,D){if(C){y(D).innerHTML=jQuery(B).datepicker("getDate").toDateString().substring(4);}else{y(D).innerHTML="pick a date";}}function s(B){$("#regions a").removeClass("current");$(B).addClass("current");}function k(B){$("#models a").removeClass("current");$(B).addClass("current");}function t(B,C,D){if((D&&e)||(!D&&o)){$(B).slideUp();$(C).removeClass("datepicker-open");}else{$(B).slideDown();$(C).addClass("datepicker-open");}if(D){e=!e;}else{o=!o;}return false;}var u={bubbleChart:function(){var D=y("topics"),C,E,B;return{draw:function(){D.innerHTML=null;var O=D.offsetWidth;var X=D.offsetHeight;B=d3.select("#topics").append("svg").attr("width",O).attr("height",X);var G=a.links;var L=d3.forceSimulation().force("link",d3.forceLink().id(function(aa){return aa.uid;}).distance(function(aa){return(1/aa.value)*20000;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(O/2,X/2));var M=B.append("g").attr("class","links").selectAll("line").data(G).enter().append("line");E={};G.forEach(function(aa){E[aa.source.uid+","+aa.target.uid]=1;});var U=a.topics;var H=U.length;var Y=U[0].score;for(var Q=0;Q<H;Q++){if(U[Q].score>Y){Y=U[Q].score;}}var W=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);C=B.selectAll(".node").data(U).enter().append("circle").attr("class","node").attr("cx",function(aa){return aa.x;}).attr("cy",function(aa){return aa.y;}).attr("r",0).style("fill",function(aa){return N(aa);}).style("stroke",function(ab,aa){return d3.rgb(ab.color).darker(2);}).call(d3.drag().on("start",R).on("drag",P).on("end",T));L.nodes(U).on("tick",K);L.force("link").links(G);function K(){M.attr("x1",function(aa){return aa.source.x;}).attr("y1",function(aa){return aa.source.y;}).attr("x2",function(aa){return aa.target.x;}).attr("y2",function(aa){return aa.target.y;});C.attr("cx",function(aa){return aa.x;}).attr("cy",function(aa){return aa.y;});}C.transition().duration(1000).attr("r",function(aa){return aa.score*500;});B.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){j();var aa=d3.select(this);aa.moveToFront();var ab=this.__data__;return'<div class="tipsy-topic">'+F(ab.nameTokens)+'</div><span class="tipsy-time">'+I((ab.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var aa=this.__data__;S(aa.docs);V(aa.years);J(aa,0.1);});function Z(ab,aa){return E[ab.uid+","+aa.uid]||E[aa.uid+","+ab.uid];}function J(ab,aa){B.selectAll("circle, line").style("opacity",aa);M.style("opacity",function(ac){return ac.source===ab||ac.target===ab?1:aa;});C.style("opacity",function(ac){return Z(ab,ac)?1:aa;});}function S(ac){var ab=y("docs");ab.innerHTML=null;listLength=ac.length;for(var aa=0;aa<listLength&&aa<6;aa++){ab.innerHTML+='<a href="'+ac[aa]+'"> Document #'+aa+"</a>";}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function V(af){var ad=new Array();for(var aj=0;aj<af.length;aj++){ad.push(new Date(af[aj],1,1));}var aa=y("tempHist");aa.innerHTML=null;B=d3.select("#tempHist").append("svg").attr("width",aa.offsetWidth*2).attr("height",aa.offsetHeight/3);var ac={top:10,right:30,bottom:30,left:30};var ab=B.attr("width")-ac.left-ac.right;var ak=B.attr("height")-ac.top-ac.bottom;var ae=B.append("g").attr("transform","translate("+ac.left+","+ac.top+")");var ai=d3.scaleTime().domain([new Date(2006,1,1),new Date(2017,1,1)]).rangeRound([0,ab]);var al=d3.histogram().domain(ai.domain()).thresholds(ai.ticks(d3.timeYear))(ad);var ah=d3.scaleLinear().domain([0,d3.max(al,function(am){return am.length;})]).range([ak,0]);var ag=ae.selectAll(".bar").data(al).enter().append("g").attr("class","bar").attr("transform",function(am){return"translate("+ai(am.x0)+","+ah(am.length)+")";});ag.append("rect").attr("x",1).attr("width",ai(al[0].x1)-ai(al[0].x0)-1).attr("height",function(am){return ak-ah(am.length);});if(ad.length>0){ae.append("g").attr("class","axisBlue").attr("transform","translate(0,"+ak+")").call(d3.axisBottom(ai));ae.append("g").attr("class","axisBlue").call(d3.axisLeft(ah));}$("#tempHist").prepend("<span>Temporal trend for the topic</span>");}function R(aa){if(!d3.event.active){L.alphaTarget(0.3).restart();}aa.fx=aa.x;aa.fy=aa.y;}function P(aa){aa.fx=d3.event.x;aa.fy=d3.event.y;}function T(aa){if(!d3.event.active){L.alphaTarget(0);}aa.fx=null;aa.fy=null;}function N(aa){aa.color=W(Math.floor(aa.score/(Y/5)));return aa.color;}function I(aa){if(aa==0){return"";}else{if(aa>59){return Math.floor(aa/60)+" h. "+I(aa%60);}else{return aa+" avg score.";}}}function F(ac){var ad=ac.length;var ab="";for(var aa=0;aa<ad;aa++){ab+=ac[aa]+" ";}return ab;}},remove:function(){C.transition().duration(1000).attr("r",0).remove();B.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();q();}};}};var h=u.bubbleChart();jQuery("#cancerBtn").click(function(){z(this,"cancer");});jQuery("#diabetesBtn").click(function(){z(this,"diabetes");});jQuery("#ldaBtn").click(function(){n(this,"LDA");});jQuery("#lldaBtn").click(function(){n(this,"LLDA");});jQuery("#dualBtn").click(function(){n(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){t("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){t("#enddatepicker","#endDatepickerBtn",false);});})();
