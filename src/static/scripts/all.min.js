 ...
 */
(function(c){function b(e,d){return(typeof e=="function")?(e.call(d)):e;}function a(e,d){this.$element=c(e);this.options=d;this.enabled=true;this.fixTitle();}a.prototype={show:function(){var l=this.getTitle();if(l&&this.enabled){var f=this.tip();f.find(".tipsy-inner")[this.options.html?"html":"text"](l);f[0].className="tipsy";f.remove().css({top:0,left:0,visibility:"hidden",display:"block"}).prependTo(document.body);var i=c.extend({},this.$element.offset(),{width:this.$element[0].offsetWidth||0,height:this.$element[0].offsetHeight||0});if(typeof this.$element[0].nearestViewportElement=="object"){var e=this.$element[0];var j=e.getBoundingClientRect();i.width=j.width;i.height=j.height;}var d=f[0].offsetWidth,g=f[0].offsetHeight,n=b(this.options.gravity,this.$element[0]);var k;switch(n.charAt(0)){case"n":k={top:i.top+i.height+this.options.offset,left:i.left+i.width/2-d/2};break;case"s":k={top:i.top-g-this.options.offset,left:i.left-this.options.offset+i.width/2-d/2};break;case"e":k={top:i.top+i.height/2-g/2,left:i.left-d-this.options.offset};break;case"w":k={top:i.top+i.height/2-g/2,left:i.left+i.width+this.options.offset};break;}if(n.length==2){if(n.charAt(1)=="w"){k.left=i.left+i.width/2-15;}else{k.left=i.left+i.width/2-d+15;}}f.css(k).addClass("tipsy-"+n);f.find(".tipsy-arrow")[0].className="tipsy-arrow tipsy-arrow-"+n.charAt(0);if(this.options.className){f.addClass(b(this.options.className,this.$element[0]));}if(this.options.fade){f.stop().css({opacity:0,display:"block",visibility:"visible"}).animate({opacity:this.options.opacity},this.options.fadeAnimationDuration);}else{f.css({visibility:"visible",opacity:this.options.opacity});}var m=this;var h=function(o){return function(){m.$tip.stop();m.tipHovered=o;if(!o){if(m.options.delayOut===0){m.hide();}else{setTimeout(function(){if(m.hoverState=="out"){m.hide();}},m.options.delayOut);}}};};f.hover(h(true),h(false));}},hide:function(){if(this.options.fade){this.tip().stop().fadeOut(this.options.fadeAnimationDuration,function(){c(this).remove();});}else{this.tip().remove();}},fixTitle:function(){var d=this.$element;if(d.attr("title")||typeof(d.attr("original-title"))!="string"){d.attr("original-title",d.attr("title")||"").removeAttr("title");}if(typeof d.context.nearestViewportElement=="object"){if(d.children("title").length){d.append("<original-title>"+(d.children("title").text()||"")+"</original-title>").children("title").remove();}}},getTitle:function(){var g,e=this.$element,f=this.options;this.fixTitle();if(typeof f.title=="string"){var d=f.title=="title"?"original-title":f.title;if(e.children(d).length){g=e.children(d).html();}else{g=e.attr(d);}}else{if(typeof f.title=="function"){g=f.title.call(e[0]);}}g=(""+g).replace(/(^\s*|\s*$)/,"");return g||f.fallback;},tip:function(){if(!this.$tip){this.$tip=c('<div class="tipsy"></div>').html('<div class="tipsy-arrow"></div><div class="tipsy-inner"></div>');}return this.$tip;},validate:function(){if(!this.$element[0].parentNode){this.hide();this.$element=null;this.options=null;}},enable:function(){this.enabled=true;},disable:function(){this.enabled=false;},toggleEnabled:function(){this.enabled=!this.enabled;}};c.fn.tipsy=function(h){if(h===true){return this.data("tipsy");}else{if(typeof h=="string"){var j=this.data("tipsy");if(j){j[h]();}return this;}}h=c.extend({},c.fn.tipsy.defaults,h);if(h.hoverlock&&h.delayOut===0){h.delayOut=100;}function g(l){var m=c.data(l,"tipsy");if(!m){m=new a(l,c.fn.tipsy.elementOptions(l,h));c.data(l,"tipsy",m);}return m;}function k(){var l=g(this);l.hoverState="in";if(h.delayIn===0){l.show();}else{l.fixTitle();setTimeout(function(){if(l.hoverState=="in"){l.show();}},h.delayIn);}}function f(){var l=g(this);l.hoverState="out";if(h.delayOut===0){l.hide();}else{var m=function(){if(!l.tipHovered||!h.hoverlock){if(l.hoverState=="out"){l.hide();}}};setTimeout(m,h.delayOut);}}if(h.trigger!="manual"){var d=h.live?"live":"bind",i=h.trigger=="hover"?"mouseenter":"focus",e=h.trigger=="hover"?"mouseleave":"blur";this[d](i,k)[d](e,f);}return this;};c.fn.tipsy.defaults={className:null,delayIn:0,delayOut:0,fade:false,fadeAnimationDuration:400,fallback:"",gravity:"n",html:false,live:false,offset:0,opacity:0.8,title:"title",trigger:"hover",hoverlock:false};c.fn.tipsy.elementOptions=function(e,d){return c.metadata?c.extend({},d,c(e).metadata()):d;};c.fn.tipsy.autoNS=function(){return c(this).offset().top>(c(document).scrollTop()+c(window).height()/2)?"s":"n";};c.fn.tipsy.autoWE=function(){return c(this).offset().left>(c(document).scrollLeft()+c(window).width()/2)?"e":"w";};c.fn.tipsy.autoBounds=function(e,d){return function(){var f={ns:d[0],ew:(d.length>1?d[1]:false)},i=c(document).scrollTop()+e,g=c(document).scrollLeft()+e,h=c(this);if(h.offset().top<i){f.ns="n";}if(h.offset().left<g){f.ew="w";}if(c(window).width()+c(document).scrollLeft()-h.offset().left<e){f.ew="e";}if(c(window).height()+c(document).scrollTop()-h.offset().top<e){f.ns="s";}return f.ns+(f.ew?f.ew:"");};};})(jQuery);
(function(){var t=560;var z=1.8;var h=10;var F=19;var j=2;var u=1;var y=1.2;var J=0.3;var a="diabetes",b="LLDA",e,D,d;var g=0,l=0,c=0,n=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(L){E("#startdatepicker","#startDatepickerBtn",true);C(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(L){E("#enddatepicker","#endDatepickerBtn",false);C(true,"#enddatepicker","endDateText");k();}});$("#startdatepicker").datepicker("setDate","01/01/2008");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");r();o();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){K("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(L){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(L){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(L){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(L){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function K(L){return document.getElementById(L);}function o(){var M=document.URL.split("/");var L=b=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";e=M[0]+"//"+M[2]+"/"+L+"?keywords="+a;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);e+="&start_date="+startDate+"&end_date="+endDate;e+="&limit="+t;if(b!="DUAL"){e+="&model="+b;}var N=new XMLHttpRequest();N.open("GET",e,true);N.onreadystatechange=f(N);N.send(null);p();r();}function f(L){return function(){if(L.readyState==4){if(L.status==200){v();w(L.responseText);}else{console.log("Retrying..");D=setTimeout(function(){console.log(e);var M=new XMLHttpRequest();M.open("GET",e,true);M.onreadystatechange=f(M);M.send(null);},60000);}}};}function w(L){try{d=JSON.parse(L);}catch(M){d=L;}if(d==""){K("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(d.topics.length==0){K("topics").innerHTML="NO TOPIC FOUND!";return;}s();m();c=0;setTimeout(B,3000);}function G(L){K("topics").innerHTML=L||"YOU CRASHED IT!";}function r(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function v(){$("#topics").empty();}function s(L){q.draw(L);}function k(){if(D!=null){clearTimeout(D);}if(q!=null){q.remove();}}function B(M,L){try{if(c){return;}if(!M){M=$("circle");}if(typeof L=="undefined"){L=0;}if(M[L]){n=$(M[L]).mouseover();}}catch(N){}}function p(){if(n){n.mouseout();c=1;n=null;}}function m(L){var L="Topics extracted from medical abstracts published";L+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(a=="diabetes"){L+=" about Diabetes";}else{L+=" about Cancer";}$("#topics").prepend("<span>"+L+"</span>");}function I(M,L){if(a!=L){a=L;k();x(M);}return false;}function H(M,L){if(b!=L){b=L;k();A(M);}return false;}function C(M,L,N){if(M){K(N).innerHTML=jQuery(L).datepicker("getDate").toDateString().substring(4);}else{K(N).innerHTML="pick a date";}}function x(L){$("#regions a").removeClass("current");$(L).addClass("current");}function A(L){$("#models a").removeClass("current");$(L).addClass("current");}function E(L,M,N){if((N&&g)||(!N&&l)){$(L).slideUp();$(M).removeClass("datepicker-open");}else{$(L).slideDown();$(M).addClass("datepicker-open");}if(N){g=!g;}else{l=!l;}return false;}var i={bubbleChart:function(){var N=K("topics"),M,O,L;return{draw:function(){N.innerHTML=null;var aa=N.offsetWidth;var aj=N.offsetHeight;L=d3.select("#topics").append("svg").attr("width",aa).attr("height",aj);var Q=d.links;var X=d3.forceSimulation().force("link",d3.forceLink().id(function(ao){return ao.uid;}).distance(function(ao){if(b=="LDA"){console.log(ao.value*u);return(1/(ao.value*u))*20000;}else{if(b=="LLDA"){console.log("weird");console.log(ao.value*J);return(1/(ao.value*J))*20000;}else{return(1/(ao.value*y))*20000;}}})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(aa/2,aj/2));var Y=L.append("g").attr("class","links").selectAll("line").data(Q).enter().append("line");O={};Q.forEach(function(ao){O[ao.source+","+ao.target]=1;});console.log(O);var ag=Object.keys(d.topics).map(function(ao){return d.topics[ao];});var S=ag.length;var ak=ag[0].score;for(var ac=0;ac<S;ac++){if(ag[ac].score>ak){ak=ag[ac].score;}}var ai=d3.scaleOrdinal().range(["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);var R=d3.scaleOrdinal().range(["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]);M=L.selectAll(".node").data(ag).enter().append("circle").attr("class","node").attr("cx",function(ao){return ao.x;}).attr("cy",function(ao){return ao.y;}).attr("r",0).style("fill",function(ao){return Z(ao);}).style("stroke",function(ap,ao){return d3.rgb(ap.color).darker(2);}).call(d3.drag().on("start",ad).on("drag",ab).on("end",af));X.nodes(ag).on("tick",W);X.force("link").links(Q);function W(){Y.attr("x1",function(ao){return ao.source.x;}).attr("y1",function(ao){return ao.source.y;}).attr("x2",function(ao){return ao.target.x;}).attr("y2",function(ao){return ao.target.y;});M.attr("cx",function(ao){return ao.x;}).attr("cy",function(ao){return ao.y;});}M.transition().duration(1000).attr("r",function(ao){if(b=="LLDA"){return ao.score*500*h;}else{if(b=="DUAL"&&ao.uid<0){return ao.score*500*(F*0.8);}else{if(b=="DUAL"){return ao.score*500*(j*0.8);}}}return ao.score*500*z;});L.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){p();var ao=d3.select(this);ao.moveToFront();var ap=this.__data__;return'<div class="tipsy-topic">'+P(ap.nameTokens)+'</div><span class="tipsy-time">'+T((ap.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var ap=this.__data__;if(b!="DUAL"){ae(ap.docs,ap.wordsProb,ap.nameTokens);if(ap.years.length>0){ah(ap.years,ap.maxYearCount,"tempHist");}else{var ao=K("tempHist");ao.innerHTML=null;$("#tempHist").prepend("<span>Not enough data</span>");}if(ap.forecastYears.length>0){ah(ap.forecastYears,ap.maxYearCount,"forTempHist");}else{var ao=K("forTempHist");ao.innerHTML=null;$("#forTempHist").prepend("<span>Not enough data to forecast</span>");}}U(ap,0.1);});function am(ap,ao){return O[ap.uid+","+ao.uid]||O[ao.uid+","+ap.uid];}function U(ap,ao){L.selectAll("circle, line").style("opacity",ao);Y.style("opacity",function(aq){return aq.source===ap||aq.target===ap?1:ao;});M.style("opacity",function(aq){return ap.uid==aq.uid||am(ap,aq)?1:ao;});}function al(aq){var ao="";for(var ap=0;ap<aq.length;ap++){ao+=aq[ap]+" ";}titleLength=ao.length<40?doc.title.length:40;return ao.slice(0,titleLength)+"...";}function an(au,at){var aq='<div id="mod'+au+'" class="modal"><div class="modal-content"><span class="close" id=clo'+au+">&times;</span><p><b>"+at.title+"</b></p>";var av=at.uiText.split(" ");aq+="<p>";for(var ao=0;ao<at.topicList.length;ao++){var ap=at.topicList[ao];aq+='<a class="modalReg" id="top'+ap+'">';aq+=al(d.topics[ap].nameTokens)+"</a>";topId="#top"+ap;$("#docs").on({mouseenter:function(ax){$(".snip").css({opacity:0.2});var aw=".sniptop"+(this.id).slice(3,(this.id).length);$(aw).css({opacity:1});},mouseleave:function(aw){$(".snip").css({opacity:1});}},topId);}aq+="</p><p>";for(var ao=0;ao<av.length;ao++){aq+='<span class="snip ';for(var ar=0;ar<at.topicList.length;ar++){var ap=at.topicList[ar];if(ao>=at.summaries[ap][0]&&ao<=at.summaries[ap][1]){aq+=" sniptop"+ap;}}aq+='">'+av[ao]+" </span>";}aq+="</p></div></div>";return aq;}function V(av,au){var ar='<div id="mod'+av+'" class="modal"><div class="modal-content"><span class="close" id=clo'+av+">&times;</span><p><b>"+au.title+"</b></p>";var ap={};var aw=au.uiText.split(" ");for(var ao=0;ao<aw.length;ao++){ap[aw[ao].replace(/\W/g,"").toLowerCase()]={topicId:"",prob:0};}ar+="<p>";for(var ao=0;ao<au.topicList.length;ao++){var aq=au.topicList[ao];ar+='<a class="region" id="top'+aq+'">';ar+=al(d.topics[aq].nameTokens)+"</a>";topId="#top"+aq;$("#docs").on({mouseenter:function(ay){$(this).attr("class","current");$(".word").css({opacity:0});var ax=".wtop"+(this.id).slice(3,(this.id).length);$(ax).css({opacity:1});$(".wtop").css({opacity:0.3});},mouseleave:function(ax){$(this).attr("class","region");$(".word").css({opacity:1});}},topId);for(var at=0;at<d.topics[aq].wordsProb.length;at++){word=d.topics[aq].wordsProb[at][0];prob=d.topics[aq].wordsProb[at][1];if((word in ap)&&d.topics[aq].score*prob>ap[word].prob){ap[word].topicId=aq;ap[word].prob=d.topics[aq].score*prob;}}}ar+="</p><p>";for(var ao=0;ao<aw.length;ao++){ar+='<span class="word wtop';key=aw[ao].replace(/\W/g,"").toLowerCase();if(key in ap){ar+=ap[key].topicId;}ar+='">'+aw[ao]+" </span>";}ar+="</p></div></div>";return ar;}function ae(av,aq,au){var ar=K("docs");ar.innerHTML=null;listLength=av.length;for(var ao=0;ao<listLength&&ao<5;ao++){pmid=av[ao];doc=d.docs[pmid];titleLength=doc.title.length<21?doc.title.length:21;title=doc.title.slice(0,titleLength)+"...";ar.innerHTML+='<button class="region" id="but'+pmid+'">'+title+"</button>";ar.innerHTML+=an(pmid,doc);var at="#but"+pmid;$("#docs").on("click",at,function(aw){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="block";});var ap="#clo"+pmid;$("#docs").on("click",ap,function(aw){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function ah(ax,ar,aC){var au=new Array();var aq=new Date(2050,1,1);var aw=new Date(1960,1,1);for(var aB=0;aB<ax.length;aB++){date=new Date(ax[aB],1,1);if(date<aq){aq=date;}if(date>aw){aw=date;}au.push(date);}var ap=K(aC);ap.innerHTML=null;L=d3.select("#"+aC).append("svg").attr("width",ap.offsetWidth*2).attr("height",ap.offsetHeight/3);var at={top:10,right:30,bottom:30,left:40};var ao=L.attr("width")-at.left-at.right;var aD=L.attr("height")-at.top-at.bottom;var av=L.append("g").attr("transform","translate("+at.left+","+at.top+")");minEndDate=new Date(aq.getFullYear()+5,1,1);aw=minEndDate.getTime()<aw.getTime()?aw:minEndDate;var aA=d3.scaleTime().domain([aq,aw]).rangeRound([0,ao]);var aE=d3.histogram().domain(aA.domain()).thresholds(aA.ticks(d3.timeYear))(au);var az=d3.scaleLinear().domain([0,ar]).range([aD,0]);var ay=av.selectAll(".bar").data(aE).enter().append("g").attr("class","bar").attr("transform",function(aF){return"translate("+aA(aF.x0)+","+az(aF.length)+")";});ay.append("rect").attr("x",1).attr("width",aA(aE[0].x1)-aA(aE[0].x0)-1).attr("height",function(aF){return aD-az(aF.length);});if(au.length>0){av.append("g").attr("class","axisBlue").attr("transform","translate(0,"+aD+")").call(d3.axisBottom(aA));av.append("g").attr("class","axisBlue").call(d3.axisLeft(az));}title="";if(aC=="tempHist"){title="Temporal trend for topic";}$("#"+aC).prepend("<span>"+title+"</span>");}function ad(ao){if(!d3.event.active){X.alphaTarget(0.3).restart();}ao.fx=ao.x;ao.fy=ao.y;}function ab(ao){ao.fx=d3.event.x;ao.fy=d3.event.y;}function af(ao){if(!d3.event.active){X.alphaTarget(0);}ao.fx=null;ao.fy=null;}function Z(ao){if(ao.uid>=0){ao.color=ai(Math.floor(ao.score/(ak/5)));}else{ao.color=R(Math.floor(ao.score/(ak/5)));}return ao.color;}function T(ao){if(ao==0){return"";}else{if(ao>59){return Math.floor(ao/60)+" h. "+T(ao%60);}else{return ao+" avg score.";}}}function P(aq){var ar=aq.length;var ap="";for(var ao=0;ao<ar;ao++){ap+=aq[ao]+" ";}return ap;}},remove:function(){if(M!=null){M.transition().duration(1000).attr("r",0).remove();}if(L!=null){L.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();}var P=K("docs");P.innerHTML=null;var P=K("tempHist");P.innerHTML=null;var P=K("forTempHist");P.innerHTML=null;o();}};}};var q=i.bubbleChart();jQuery("#cancerBtn").click(function(){I(this,"cancer");});jQuery("#diabetesBtn").click(function(){I(this,"diabetes");});jQuery("#ldaBtn").click(function(){H(this,"LDA");});jQuery("#lldaBtn").click(function(){H(this,"LLDA");});jQuery("#dualBtn").click(function(){H(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){E("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){E("#enddatepicker","#endDatepickerBtn",false);});})();
