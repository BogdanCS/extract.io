...
(function(){var l=10;var x="diabetes",d="LLDA",a;var e=0,o=0,w=0,c=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#startdatepicker","#startDatepickerBtn",true);p(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(B){t("#enddatepicker","#endDatepickerBtn",false);p(true,"#enddatepicker","endDateText");m();}});$("#startdatepicker").datepicker("setDate","01/01/2001");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();q();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){y("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(B){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(B){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function y(B){return document.getElementById(B);}function q(){var D=document.URL.split("/");var B=d=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";var C=D[0]+"//"+D[2]+"/"+B+"?keywords="+x;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);C+="&start_date="+startDate+"&end_date="+endDate;C+="&limit="+l;if(d!="DUAL"){C+="&model="+d;}var E=new XMLHttpRequest();E.open("GET",C,true);E.onreadystatechange=function(){if(E.readyState==4){g();if(E.status==200){f(E.responseText);}else{v();}}};E.send(null);j();b();}function f(B){try{a=JSON.parse(B);}catch(C){a=B;}if(a==""){y("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){y("topics").innerHTML="NO TOPIC FOUND!";return;}r();i();w=0;setTimeout(A,3000);}function v(B){y("topics").innerHTML=B||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function g(){$("#topics").empty();}function r(B){h.draw(B);}function m(){h.remove();}function A(C,B){try{if(w){return;}if(!C){C=$("circle");}if(typeof B=="undefined"){B=0;}if(C[B]){c=$(C[B]).mouseover();}}catch(D){}}function j(){if(c){c.mouseout();w=1;c=null;}}function i(B){var B="Topics extracted from medical abstracts published";B+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(x=="diabetes"){B+=" about Diabetes";}else{B+=" about Cancer";}$("#topics").prepend("<span>"+B+"</span>");}function z(C,B){if(x!=B){x=B;m();s(C);}return false;}function n(C,B){if(d!=B){d=B;m();k(C);}return false;}function p(C,B,D){if(C){y(D).innerHTML=jQuery(B).datepicker("getDate").toDateString().substring(4);}else{y(D).innerHTML="pick a date";}}function s(B){$("#regions a").removeClass("current");$(B).addClass("current");}function k(B){$("#models a").removeClass("current");$(B).addClass("current");}function t(B,C,D){if((D&&e)||(!D&&o)){$(B).slideUp();$(C).removeClass("datepicker-open");}else{$(B).slideDown();$(C).addClass("datepicker-open");}if(D){e=!e;}else{o=!o;}return false;}var u={bubbleChart:function(){var D=y("topics"),C,E,B;return{draw:function(){D.innerHTML=null;var P=D.offsetWidth;var Y=D.offsetHeight;B=d3.select("#topics").append("svg").attr("width",P).attr("height",Y);var G=a.links;var M=d3.forceSimulation().force("link",d3.forceLink().id(function(ac){return ac.uid;}).distance(function(ac){return(1/ac.value)*20000;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(P/2,Y/2));var N=B.append("g").attr("class","links").selectAll("line").data(G).enter().append("line");E={};G.forEach(function(ac){E[ac.source.uid+","+ac.target.uid]=1;});var V=Object.keys(a.topics).map(function(ac){return a.topics[ac];});var H=V.length;var Z=V[0].score;for(var R=0;R<H;R++){if(V[R].score>Z){Z=V[R].score;}}var X=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);C=B.selectAll(".node").data(V).enter().append("circle").attr("class","node").attr("cx",function(ac){return ac.x;}).attr("cy",function(ac){return ac.y;}).attr("r",0).style("fill",function(ac){return O(ac);}).style("stroke",function(ad,ac){return d3.rgb(ad.color).darker(2);}).call(d3.drag().on("start",S).on("drag",Q).on("end",U));M.nodes(V).on("tick",L);M.force("link").links(G);function L(){N.attr("x1",function(ac){return ac.source.x;}).attr("y1",function(ac){return ac.source.y;}).attr("x2",function(ac){return ac.target.x;}).attr("y2",function(ac){return ac.target.y;});C.attr("cx",function(ac){return ac.x;}).attr("cy",function(ac){return ac.y;});}C.transition().duration(1000).attr("r",function(ac){return ac.score*500;});B.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){j();var ac=d3.select(this);ac.moveToFront();var ad=this.__data__;return'<div class="tipsy-topic">'+F(ad.nameTokens)+'</div><span class="tipsy-time">'+I((ad.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var ad=this.__data__;T(ad.docs,ad.wordsProb,ad.nameTokens);if(ad.years.length>0){W(ad.years,ad.maxYearCount,"tempHist");}else{var ac=y("tempHist");ac.innerHTML=null;$("#tempHist").prepend("<span>Not enough data</span>");}if(ad.forecastYears.length>0){W(ad.forecastYears,ad.maxYearCount,"forTempHist");}else{var ac=y("forTempHist");ac.innerHTML=null;$("#forTempHist").prepend("<span>Not enough data to forecast</span>");}J(ad,0.1);});function ab(ad,ac){return E[ad.uid+","+ac.uid]||E[ac.uid+","+ad.uid];}function J(ad,ac){B.selectAll("circle, line").style("opacity",ac);N.style("opacity",function(ae){return ae.source===ad||ae.target===ad?1:ac;});C.style("opacity",function(ae){return ab(ad,ae)?1:ac;});}function aa(ae){var ac="";for(var ad=0;ad<ae.length;ad++){ac+=ae[ad]+" ";}titleLength=ac.length<11?doc.title.length:11;return ac.slice(0,titleLength)+"...";}function K(ai,ah){var af='<div id="mod'+ai+'" class="modal"><div class="modal-content"><span class="close" id=clo'+ai+">&times;</span><p><b>"+ah.title+"</b></p>";var ad={};var aj=ah.uiText.split(" ");for(var ac=0;ac<aj.length;ac++){ad[aj[ac].replace(/\W/g,"").toLowerCase()]={topicId:"",prob:0};}af+="<p>";for(var ac=0;ac<ah.topicList.length;ac++){var ae=ah.topicList[ac];af+='<a class="region" id="top'+ae+'">';af+=aa(a.topics[ae].nameTokens)+"</a>";topId="#top"+ae;$("#docs").on({mouseenter:function(al){$(this).attr("class","current");$(".word").css({opacity:0});var ak=".wtop"+(this.id).slice(3,(this.id).length);$(ak).css({opacity:1});$(".wtop").css({opacity:0.3});},mouseleave:function(ak){$(this).attr("class","region");$(".word").css({opacity:1});}},topId);for(var ag=0;ag<a.topics[ae].wordsProb.length;ag++){word=a.topics[ae].wordsProb[ag][0];prob=a.topics[ae].wordsProb[ag][1];if((word in ad)&&a.topics[ae].score*prob>ad[word].prob){ad[word].topicId=ae;ad[word].prob=a.topics[ae].score*prob;}}}af+="</p><p>";for(var ac=0;ac<aj.length;ac++){af+='<span class="word wtop';key=aj[ac].replace(/\W/g,"").toLowerCase();if(key in ad){af+=ad[key].topicId;}af+='">'+aj[ac]+" </span>";}af+="</p></div></div>";return af;}function T(ai,ae,ah){var af=y("docs");af.innerHTML=null;listLength=ai.length;for(var ac=0;ac<listLength&&ac<5;ac++){pmid=ai[ac];doc=a.docs[pmid];titleLength=doc.title.length<21?doc.title.length:21;title=doc.title.slice(0,titleLength)+"...";af.innerHTML+='<button class="region" id="but'+pmid+'">'+title+"</button>";af.innerHTML+=K(pmid,doc);var ag="#but"+pmid;$("#docs").on("click",ag,function(aj){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="block";});var ad="#clo"+pmid;$("#docs").on("click",ad,function(aj){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function W(ak,af,ap){var ah=new Array();var ae=new Date(2050,1,1);var aj=new Date(1960,1,1);for(var ao=0;ao<ak.length;ao++){date=new Date(ak[ao],1,1);if(date<ae){ae=date;}if(date>aj){aj=date;}ah.push(date);}var ad=y(ap);ad.innerHTML=null;B=d3.select("#"+ap).append("svg").attr("width",ad.offsetWidth*2).attr("height",ad.offsetHeight/3);var ag={top:10,right:30,bottom:30,left:40};var ac=B.attr("width")-ag.left-ag.right;var aq=B.attr("height")-ag.top-ag.bottom;var ai=B.append("g").attr("transform","translate("+ag.left+","+ag.top+")");minEndDate=new Date(ae.getFullYear()+5,1,1);aj=minEndDate.getTime()<aj.getTime()?aj:minEndDate;var an=d3.scaleTime().domain([ae,aj]).rangeRound([0,ac]);var ar=d3.histogram().domain(an.domain()).thresholds(an.ticks(d3.timeYear))(ah);var am=d3.scaleLinear().domain([0,af]).range([aq,0]);var al=ai.selectAll(".bar").data(ar).enter().append("g").attr("class","bar").attr("transform",function(at){return"translate("+an(at.x0)+","+am(at.length)+")";});al.append("rect").attr("x",1).attr("width",an(ar[0].x1)-an(ar[0].x0)-1).attr("height",function(at){return aq-am(at.length);});if(ah.length>0){ai.append("g").attr("class","axisBlue").attr("transform","translate(0,"+aq+")").call(d3.axisBottom(an));ai.append("g").attr("class","axisBlue").call(d3.axisLeft(am));}title="";if(ap=="tempHist"){title="Temporal trend for topic";}$("#"+ap).prepend("<span>"+title+"</span>");}function S(ac){if(!d3.event.active){M.alphaTarget(0.3).restart();}ac.fx=ac.x;ac.fy=ac.y;}function Q(ac){ac.fx=d3.event.x;ac.fy=d3.event.y;}function U(ac){if(!d3.event.active){M.alphaTarget(0);}ac.fx=null;ac.fy=null;}function O(ac){ac.color=X(Math.floor(ac.score/(Z/5)));return ac.color;}function I(ac){if(ac==0){return"";}else{if(ac>59){return Math.floor(ac/60)+" h. "+I(ac%60);}else{return ac+" avg score.";}}}function F(ae){var af=ae.length;var ad="";for(var ac=0;ac<af;ac++){ad+=ae[ac]+" ";}return ad;}},remove:function(){C.transition().duration(1000).attr("r",0).remove();B.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();var F=y("docs");F.innerHTML=null;var F=y("tempHist");F.innerHTML=null;var F=y("forTempHist");F.innerHTML=null;q();}};}};var h=u.bubbleChart();jQuery("#cancerBtn").click(function(){z(this,"cancer");});jQuery("#diabetesBtn").click(function(){z(this,"diabetes");});jQuery("#ldaBtn").click(function(){n(this,"LDA");});jQuery("#lldaBtn").click(function(){n(this,"LLDA");});jQuery("#dualBtn").click(function(){n(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){t("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){t("#enddatepicker","#endDatepickerBtn",false);});})();
