...
(function(){var j=500;var u="diabetes",a;var d=0,l=0,t=0,c=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(757382400),maxDate:new Date(),onSelect:function(y){q("#startdatepicker","#startDatepickerBtn",true);m(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(757382400),maxDate:new Date(),onSelect:function(y){q("#enddatepicker","#endDatepickerBtn",false);m(true,"#enddatepicker","endDateText");k();}});$("#startdatepicker").datepicker("setDate",new Date());$("#enddatepicker").datepicker("setDate",new Date());$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();n();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){v("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(y){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(y){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(y){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(y){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function v(y){return document.getElementById(y);}function n(){console.log("got here");var z=document.URL.split("/");var y=z[0]+"//"+z[2]+"/rpcNewSearch?keywords="+u;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);y+="&start_date="+startDate+"&end_date="+endDate;y+="&limit="+j;var A=new XMLHttpRequest();A.open("GET",y,true);A.onreadystatechange=function(){if(A.readyState==4){f();if(A.status==200){e(A.responseText);}else{s();}}};A.send(null);i();b();}function e(y){try{a=JSON.parse(y);}catch(z){a=y;}if(a==""){v("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){v("topics").innerHTML="NO TOPIC FOUND!";return;}o();h();t=0;setTimeout(x,3000);}function s(y){v("topics").innerHTML=y||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function f(){$("#topics").empty();}function o(y){g.draw(y);}function k(){g.remove();}function x(z,y){try{if(t){return;}if(!z){z=$("circle");}if(typeof y=="undefined"){y=0;}if(z[y]){c=$(z[y]).mouseover();setTimeout(function(){$(z[y]).mouseout();c=null;x(z,y+1);},5000);}}catch(A){}}function i(){if(c){c.mouseout();t=1;c=null;}}function h(y){var y="Topics extracted from medical abstracts published";y+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(u=="diabetes"){y+=" about Diabetes";}else{y+=" about Cancer";}$("#topics").prepend("<span>"+y+"</span>");}function w(z,y){if(u!=y){u=y;k();p(z);}return false;}function m(z,y,A){if(z){v(A).innerHTML=jQuery(y).datepicker("getDate").toDateString().substring(4);}else{v(A).innerHTML="pick a date";}}function p(y){$("nav a").removeClass("current");$(y).addClass("current");}function q(y,z,A){if((A&&d)||(!A&&l)){$(y).slideUp();$(z).removeClass("datepicker-open");}else{$(y).slideDown();$(z).addClass("datepicker-open");}if(A){d=!d;}else{l=!l;}return false;}var r={bubbleChart:function(){var A=v("topics"),z,B,y;return{draw:function(){A.innerHTML=null;var L=A.offsetWidth;var U=A.offsetHeight;y=d3.select("#topics").append("svg").attr("width",L).attr("height",U);var D=a.links;var I=d3.forceSimulation().force("link",d3.forceLink().id(function(X){return X.uid;}).distance(function(X){return(1/X.value)*20000;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(L/2,U/2));var J=y.append("g").attr("class","links").selectAll("line").data(D).enter().append("line");B={};D.forEach(function(X){B[X.source.uid+","+X.target.uid]=1;});var R=a.topics;var E=R.length;var V=R[0].score;for(var N=0;N<E;N++){if(R[N].score>V){V=R[N].score;}}var T=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);z=y.selectAll(".node").data(R).enter().append("circle").attr("class","node").attr("cx",function(X){return X.x;}).attr("cy",function(X){return X.y;}).attr("r",0).style("fill",function(X){return K(X);}).style("stroke",function(Y,X){return d3.rgb(Y.color).darker(2);}).call(d3.drag().on("start",O).on("drag",M).on("end",Q));I.nodes(R).on("tick",H);I.force("link").links(D);function H(){J.attr("x1",function(X){return X.source.x;}).attr("y1",function(X){return X.source.y;}).attr("x2",function(X){return X.target.x;}).attr("y2",function(X){return X.target.y;});z.attr("cx",function(X){return X.x;}).attr("cy",function(X){return X.y;});}z.transition().duration(1000).attr("r",function(X){return X.score*500;});y.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){i();var X=d3.select(this);X.moveToFront();var Y=this.__data__;return'<div class="tipsy-topic">'+C(Y.words)+'</div><span class="tipsy-time">'+F((Y.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var X=this.__data__;P(X.docs);S(X.years);G(X,0.1);});function W(Y,X){return B[Y.uid+","+X.uid]||B[X.uid+","+Y.uid];}function G(Y,X){y.selectAll("circle, line").style("opacity",X);J.style("opacity",function(Z){return Z.source===Y||Z.target===Y?1:X;});z.style("opacity",function(Z){return W(Y,Z)?1:X;});}function P(Z){var Y=v("docs");Y.innerHTML=null;listLength=Z.length;for(var X=0;X<listLength&&X<6;X++){Y.innerHTML+='<a href="'+Z[X]+'"> Document #'+X+"</a>";}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function S(ac){var aa=new Array();for(var ag=0;ag<ac.length;ag++){aa.push(new Date(ac[ag],1,1));}var X=v("tempHist");X.innerHTML=null;y=d3.select("#tempHist").append("svg").attr("width",X.offsetWidth*2).attr("height",X.offsetHeight/3);var Z={top:10,right:30,bottom:30,left:30};var Y=y.attr("width")-Z.left-Z.right;var ah=y.attr("height")-Z.top-Z.bottom;var ab=y.append("g").attr("transform","translate("+Z.left+","+Z.top+")");var af=d3.scaleTime().domain([new Date(2006,1,1),new Date(2017,1,1)]).rangeRound([0,Y]);var ai=d3.histogram().domain(af.domain()).thresholds(af.ticks(d3.timeYear))(aa);var ae=d3.scaleLinear().domain([0,d3.max(ai,function(aj){return aj.length;})]).range([ah,0]);var ad=ab.selectAll(".bar").data(ai).enter().append("g").attr("class","bar").attr("transform",function(aj){return"translate("+af(aj.x0)+","+ae(aj.length)+")";});ad.append("rect").attr("x",1).attr("width",af(ai[0].x1)-af(ai[0].x0)-1).attr("height",function(aj){return ah-ae(aj.length);});if(aa.length>0){ab.append("g").attr("class","axisBlue").attr("transform","translate(0,"+ah+")").call(d3.axisBottom(af));ab.append("g").attr("class","axisBlue").call(d3.axisLeft(ae));}$("#tempHist").prepend("<span>Temporal trend for the topic</span>");}function O(X){if(!d3.event.active){I.alphaTarget(0.3).restart();}X.fx=X.x;X.fy=X.y;}function M(X){X.fx=d3.event.x;X.fy=d3.event.y;}function Q(X){if(!d3.event.active){I.alphaTarget(0);}X.fx=null;X.fy=null;}function K(X){X.color=T(Math.floor(X.score/(V/5)));return X.color;}function F(X){if(X==0){return"";}else{if(X>59){return Math.floor(X/60)+" h. "+F(X%60);}else{return X+" avg score.";}}}function C(Z){var aa=Z.length;var Y="";for(var X=0;X<aa;X++){Y+=Z[X]+" ";}return Y;}},remove:function(){z.transition().duration(1000).attr("r",0).remove();y.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();n();}};}};var g=r.bubbleChart();jQuery("#cancerBtn").click(function(){w(this,"cancer");});jQuery("#diabetesBtn").click(function(){w(this,"diabetes");});jQuery("#startDatepickerBtn").click(function(){q("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){q("#enddatepicker","#endDatepickerBtn",false);});})();
