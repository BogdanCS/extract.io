...
(function(){var n=50;var A="diabetes",d="DUAL",f,i,a;var e=0,q=0,z=0,c=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(E){w("#startdatepicker","#startDatepickerBtn",true);r(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(E){w("#enddatepicker","#endDatepickerBtn",false);r(true,"#enddatepicker","endDateText");o();}});$("#startdatepicker").datepicker("setDate","01/01/2001");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();s();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){B("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(E){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(E){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(E){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(E){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function B(E){return document.getElementById(E);}function s(){var F=document.URL.split("/");var E=d=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";f=F[0]+"//"+F[2]+"/"+E+"?keywords="+A;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);f+="&start_date="+startDate+"&end_date="+endDate;f+="&limit="+n;if(d!="DUAL"){f+="&model="+d;}var G=new XMLHttpRequest();G.open("GET",f,true);G.onreadystatechange=u(G);G.send(null);l();b();}function u(E){return function(){if(E.readyState==4){if(E.status==200){h();g(E.responseText);}else{console.log("Retrying..");i=setTimeout(function(){console.log(f);var F=new XMLHttpRequest();F.open("GET",f,true);F.onreadystatechange=u(F);F.send(null);},60000);}}};}function g(E){try{a=JSON.parse(E);}catch(F){a=E;}if(a==""){B("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){B("topics").innerHTML="NO TOPIC FOUND!";return;}t();k();z=0;setTimeout(D,3000);}function y(E){B("topics").innerHTML=E||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function h(){$("#topics").empty();}function t(E){j.draw(E);}function o(){if(i!=null){clearTimeout(i);}if(j!=null){j.remove();}}function D(F,E){try{if(z){return;}if(!F){F=$("circle");}if(typeof E=="undefined"){E=0;}if(F[E]){c=$(F[E]).mouseover();}}catch(G){}}function l(){if(c){c.mouseout();z=1;c=null;}}function k(E){var E="Topics extracted from medical abstracts published";E+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(A=="diabetes"){E+=" about Diabetes";}else{E+=" about Cancer";}$("#topics").prepend("<span>"+E+"</span>");}function C(F,E){if(A!=E){A=E;o();v(F);}return false;}function p(F,E){if(d!=E){d=E;o();m(F);}return false;}function r(F,E,G){if(F){B(G).innerHTML=jQuery(E).datepicker("getDate").toDateString().substring(4);}else{B(G).innerHTML="pick a date";}}function v(E){$("#regions a").removeClass("current");$(E).addClass("current");}function m(E){$("#models a").removeClass("current");$(E).addClass("current");}function w(E,F,G){if((G&&e)||(!G&&q)){$(E).slideUp();$(F).removeClass("datepicker-open");}else{$(E).slideDown();$(F).addClass("datepicker-open");}if(G){e=!e;}else{q=!q;}return false;}var x={bubbleChart:function(){var G=B("topics"),F,H,E;return{draw:function(){G.innerHTML=null;var S=G.offsetWidth;var ab=G.offsetHeight;E=d3.select("#topics").append("svg").attr("width",S).attr("height",ab);var J=a.links;for(var U=0;U<J.length;U++){console.log(J[U]);}var P=d3.forceSimulation().force("link",d3.forceLink().id(function(af){return af.uid;}).distance(function(af){return(1/af.value)*20000;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(S/2,ab/2));var Q=E.append("g").attr("class","links").selectAll("line").data(J).enter().append("line");H={};J.forEach(function(af){H[af.source.uid+","+af.target.uid]=1;});var Y=Object.keys(a.topics).map(function(af){return a.topics[af];});var K=Y.length;var ac=Y[0].score;for(var U=0;U<K;U++){if(Y[U].score>ac){ac=Y[U].score;}}var aa=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);F=E.selectAll(".node").data(Y).enter().append("circle").attr("class","node").attr("cx",function(af){return af.x;}).attr("cy",function(af){return af.y;}).attr("r",0).style("fill",function(af){return R(af);}).style("stroke",function(ag,af){return d3.rgb(ag.color).darker(2);}).call(d3.drag().on("start",V).on("drag",T).on("end",X));P.nodes(Y).on("tick",O);P.force("link").links(J);function O(){Q.attr("x1",function(af){return af.source.x;}).attr("y1",function(af){return af.source.y;}).attr("x2",function(af){return af.target.x;}).attr("y2",function(af){return af.target.y;});F.attr("cx",function(af){return af.x;}).attr("cy",function(af){return af.y;});}F.transition().duration(1000).attr("r",function(af){return af.score*500;});E.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){l();var af=d3.select(this);af.moveToFront();var ag=this.__data__;return'<div class="tipsy-topic">'+I(ag.nameTokens)+'</div><span class="tipsy-time">'+L((ag.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var ag=this.__data__;if(d!="DUAL"){W(ag.docs,ag.wordsProb,ag.nameTokens);if(ag.years.length>0){Z(ag.years,ag.maxYearCount,"tempHist");}else{var af=B("tempHist");af.innerHTML=null;$("#tempHist").prepend("<span>Not enough data</span>");}if(ag.forecastYears.length>0){Z(ag.forecastYears,ag.maxYearCount,"forTempHist");}else{var af=B("forTempHist");af.innerHTML=null;$("#forTempHist").prepend("<span>Not enough data to forecast</span>");}}M(ag,0.1);});function ae(ag,af){return H[ag.uid+","+af.uid]||H[af.uid+","+ag.uid];}function M(ag,af){E.selectAll("circle, line").style("opacity",af);Q.style("opacity",function(ah){return ah.source===ag||ah.target===ag?1:af;});F.style("opacity",function(ah){return ae(ag,ah)?1:af;});}function ad(ah){var af="";for(var ag=0;ag<ah.length;ag++){af+=ah[ag]+" ";}titleLength=af.length<11?doc.title.length:11;return af.slice(0,titleLength)+"...";}function N(al,ak){var ai='<div id="mod'+al+'" class="modal"><div class="modal-content"><span class="close" id=clo'+al+">&times;</span><p><b>"+ak.title+"</b></p>";var ag={};var am=ak.uiText.split(" ");for(var af=0;af<am.length;af++){ag[am[af].replace(/\W/g,"").toLowerCase()]={topicId:"",prob:0};}ai+="<p>";for(var af=0;af<ak.topicList.length;af++){var ah=ak.topicList[af];ai+='<a class="region" id="top'+ah+'">';ai+=ad(a.topics[ah].nameTokens)+"</a>";topId="#top"+ah;$("#docs").on({mouseenter:function(ao){$(this).attr("class","current");$(".word").css({opacity:0});var an=".wtop"+(this.id).slice(3,(this.id).length);$(an).css({opacity:1});$(".wtop").css({opacity:0.3});},mouseleave:function(an){$(this).attr("class","region");$(".word").css({opacity:1});}},topId);for(var aj=0;aj<a.topics[ah].wordsProb.length;aj++){word=a.topics[ah].wordsProb[aj][0];prob=a.topics[ah].wordsProb[aj][1];if((word in ag)&&a.topics[ah].score*prob>ag[word].prob){ag[word].topicId=ah;ag[word].prob=a.topics[ah].score*prob;}}}ai+="</p><p>";for(var af=0;af<am.length;af++){ai+='<span class="word wtop';key=am[af].replace(/\W/g,"").toLowerCase();if(key in ag){ai+=ag[key].topicId;}ai+='">'+am[af]+" </span>";}ai+="</p></div></div>";return ai;}function W(al,ah,ak){var ai=B("docs");ai.innerHTML=null;listLength=al.length;for(var af=0;af<listLength&&af<5;af++){pmid=al[af];doc=a.docs[pmid];titleLength=doc.title.length<21?doc.title.length:21;title=doc.title.slice(0,titleLength)+"...";ai.innerHTML+='<button class="region" id="but'+pmid+'">'+title+"</button>";ai.innerHTML+=N(pmid,doc);var aj="#but"+pmid;$("#docs").on("click",aj,function(am){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="block";});var ag="#clo"+pmid;$("#docs").on("click",ag,function(am){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function Z(an,ai,at){var ak=new Array();var ah=new Date(2050,1,1);var am=new Date(1960,1,1);for(var ar=0;ar<an.length;ar++){date=new Date(an[ar],1,1);if(date<ah){ah=date;}if(date>am){am=date;}ak.push(date);}var ag=B(at);ag.innerHTML=null;E=d3.select("#"+at).append("svg").attr("width",ag.offsetWidth*2).attr("height",ag.offsetHeight/3);var aj={top:10,right:30,bottom:30,left:40};var af=E.attr("width")-aj.left-aj.right;var au=E.attr("height")-aj.top-aj.bottom;var al=E.append("g").attr("transform","translate("+aj.left+","+aj.top+")");minEndDate=new Date(ah.getFullYear()+5,1,1);am=minEndDate.getTime()<am.getTime()?am:minEndDate;var aq=d3.scaleTime().domain([ah,am]).rangeRound([0,af]);var av=d3.histogram().domain(aq.domain()).thresholds(aq.ticks(d3.timeYear))(ak);var ap=d3.scaleLinear().domain([0,ai]).range([au,0]);var ao=al.selectAll(".bar").data(av).enter().append("g").attr("class","bar").attr("transform",function(aw){return"translate("+aq(aw.x0)+","+ap(aw.length)+")";});ao.append("rect").attr("x",1).attr("width",aq(av[0].x1)-aq(av[0].x0)-1).attr("height",function(aw){return au-ap(aw.length);});if(ak.length>0){al.append("g").attr("class","axisBlue").attr("transform","translate(0,"+au+")").call(d3.axisBottom(aq));al.append("g").attr("class","axisBlue").call(d3.axisLeft(ap));}title="";if(at=="tempHist"){title="Temporal trend for topic";}$("#"+at).prepend("<span>"+title+"</span>");}function V(af){if(!d3.event.active){P.alphaTarget(0.3).restart();}af.fx=af.x;af.fy=af.y;}function T(af){af.fx=d3.event.x;af.fy=d3.event.y;}function X(af){if(!d3.event.active){P.alphaTarget(0);}af.fx=null;af.fy=null;}function R(af){af.color=aa(Math.floor(af.score/(ac/5)));return af.color;}function L(af){if(af==0){return"";}else{if(af>59){return Math.floor(af/60)+" h. "+L(af%60);}else{return af+" avg score.";}}}function I(ah){var ai=ah.length;var ag="";for(var af=0;af<ai;af++){ag+=ah[af]+" ";}return ag;}},remove:function(){if(F!=null){F.transition().duration(1000).attr("r",0).remove();}if(E!=null){E.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();}var I=B("docs");I.innerHTML=null;var I=B("tempHist");I.innerHTML=null;var I=B("forTempHist");I.innerHTML=null;s();}};}};var j=x.bubbleChart();jQuery("#cancerBtn").click(function(){C(this,"cancer");});jQuery("#diabetesBtn").click(function(){C(this,"diabetes");});jQuery("#ldaBtn").click(function(){p(this,"LDA");});jQuery("#lldaBtn").click(function(){p(this,"LLDA");});jQuery("#dualBtn").click(function(){p(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){w("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){w("#enddatepicker","#endDatepickerBtn",false);});})();
