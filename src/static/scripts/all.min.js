...
(function(){var q=50;var r=1.5;var u=10;var c=1.2;var o=1.2;var j=120;var F="diabetes",e="LLDA",g,k,a;var f=0,v=0,E=0,d=null;window.onload=function(){$("#startdatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(J){B("#startdatepicker","#startDatepickerBtn",true);w(true,"#startdatepicker","startDateText");}});$("#enddatepicker").datepicker({dateFormat:"dd/mm/yy",minDate:"01/01/1994",onSelect:function(J){B("#enddatepicker","#endDatepickerBtn",false);w(true,"#enddatepicker","endDateText");s();}});$("#startdatepicker").datepicker("setDate","01/01/2001");$("#enddatepicker").datepicker("setDate","01/01/2016");$("#startDatepickerBtn").addClass("active");$("#endDatepickerBtn").addClass("active");b();x();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){G("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(J){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(J){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(J){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(J){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function G(J){return document.getElementById(J);}function x(){var K=document.URL.split("/");var J=e=="DUAL"?"rpcNewSearchDualView":"rpcNewSearch";g=K[0]+"//"+K[2]+"/"+J+"?keywords="+F;startDate=Math.floor(jQuery("#startdatepicker").datepicker("getDate").getTime()/1000);endDate=Math.floor(jQuery("#enddatepicker").datepicker("getDate").getTime()/1000);g+="&start_date="+startDate+"&end_date="+endDate;g+="&limit="+q;if(e!="DUAL"){g+="&model="+e;}var L=new XMLHttpRequest();L.open("GET",g,true);L.onreadystatechange=z(L);L.send(null);n();b();}function z(J){return function(){if(J.readyState==4){if(J.status==200){i();h(J.responseText);}else{console.log("Retrying..");k=setTimeout(function(){console.log(g);var K=new XMLHttpRequest();K.open("GET",g,true);K.onreadystatechange=z(K);K.send(null);},60000);}}};}function h(J){try{a=JSON.parse(J);}catch(K){a=J;}if(a==""){G("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){G("topics").innerHTML="NO TOPIC FOUND!";return;}y();m();E=0;setTimeout(I,3000);}function D(J){G("topics").innerHTML=J||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function i(){$("#topics").empty();}function y(J){l.draw(J);}function s(){if(k!=null){clearTimeout(k);}if(l!=null){l.remove();}}function I(K,J){try{if(E){return;}if(!K){K=$("circle");}if(typeof J=="undefined"){J=0;}if(K[J]){d=$(K[J]).mouseover();}}catch(L){}}function n(){if(d){d.mouseout();E=1;d=null;}}function m(J){var J="Topics extracted from medical abstracts published";J+=" between "+jQuery("#startdatepicker").datepicker("getDate").toDateString().substring(4)+" and "+jQuery("#enddatepicker").datepicker("getDate").toDateString().substring(4);if(F=="diabetes"){J+=" about Diabetes";}else{J+=" about Cancer";}$("#topics").prepend("<span>"+J+"</span>");}function H(K,J){if(F!=J){F=J;s();A(K);}return false;}function t(K,J){if(e!=J){e=J;s();p(K);}return false;}function w(K,J,L){if(K){G(L).innerHTML=jQuery(J).datepicker("getDate").toDateString().substring(4);}else{G(L).innerHTML="pick a date";}}function A(J){$("#regions a").removeClass("current");$(J).addClass("current");}function p(J){$("#models a").removeClass("current");$(J).addClass("current");}function B(J,K,L){if((L&&f)||(!L&&v)){$(J).slideUp();$(K).removeClass("datepicker-open");}else{$(J).slideDown();$(K).addClass("datepicker-open");}if(L){f=!f;}else{v=!v;}return false;}var C={bubbleChart:function(){var L=G("topics"),K,M,J;return{draw:function(){L.innerHTML=null;var Y=L.offsetWidth;var ah=L.offsetHeight;J=d3.select("#topics").append("svg").attr("width",Y).attr("height",ah);var O=a.links;var V=d3.forceSimulation().force("link",d3.forceLink().id(function(al){return al.uid;}).distance(function(al){if(e=="LDA"){console.log(al.value*c);return(1/(al.value*c))*20000;}else{if(e=="LLDA"){console.log("weird");console.log(al.value*j);return(1/(al.value*j))*20000;}else{return(1/(al.value*o))*20000;}}})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(Y/2,ah/2));var W=J.append("g").attr("class","links").selectAll("line").data(O).enter().append("line");M={};O.forEach(function(al){M[al.source+","+al.target]=1;});console.log(M);var ae=Object.keys(a.topics).map(function(al){return a.topics[al];});var Q=ae.length;var ai=ae[0].score;for(var aa=0;aa<Q;aa++){if(ae[aa].score>ai){ai=ae[aa].score;}}var ag=d3.scaleOrdinal().range(["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);var P=d3.scaleOrdinal().range(["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]);K=J.selectAll(".node").data(ae).enter().append("circle").attr("class","node").attr("cx",function(al){return al.x;}).attr("cy",function(al){return al.y;}).attr("r",0).style("fill",function(al){return X(al);}).style("stroke",function(am,al){return d3.rgb(am.color).darker(2);}).call(d3.drag().on("start",ab).on("drag",Z).on("end",ad));V.nodes(ae).on("tick",U);V.force("link").links(O);function U(){W.attr("x1",function(al){return al.source.x;}).attr("y1",function(al){return al.source.y;}).attr("x2",function(al){return al.target.x;}).attr("y2",function(al){return al.target.y;});K.attr("cx",function(al){return al.x;}).attr("cy",function(al){return al.y;});}K.transition().duration(1000).attr("r",function(al){if(e=="LLDA"){return al.score*500*u;}else{if(e=="DUAL"&&al.uid<0){return al.score*500*(u*0.8);}}return al.score*500*r;});J.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){n();var al=d3.select(this);al.moveToFront();var am=this.__data__;return'<div class="tipsy-topic">'+N(am.nameTokens)+'</div><span class="tipsy-time">'+R((am.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var am=this.__data__;if(e!="DUAL"){ac(am.docs,am.wordsProb,am.nameTokens);if(am.years.length>0){af(am.years,am.maxYearCount,"tempHist");}else{var al=G("tempHist");al.innerHTML=null;$("#tempHist").prepend("<span>Not enough data</span>");}if(am.forecastYears.length>0){af(am.forecastYears,am.maxYearCount,"forTempHist");}else{var al=G("forTempHist");al.innerHTML=null;$("#forTempHist").prepend("<span>Not enough data to forecast</span>");}}S(am,0.1);});function ak(am,al){return M[am.uid+","+al.uid]||M[al.uid+","+am.uid];}function S(am,al){J.selectAll("circle, line").style("opacity",al);W.style("opacity",function(an){return an.source===am||an.target===am?1:al;});K.style("opacity",function(an){return am.uid==an.uid||ak(am,an)?1:al;});}function aj(an){var al="";for(var am=0;am<an.length;am++){al+=an[am]+" ";}titleLength=al.length<11?doc.title.length:11;return al.slice(0,titleLength)+"...";}function T(ar,aq){var ao='<div id="mod'+ar+'" class="modal"><div class="modal-content"><span class="close" id=clo'+ar+">&times;</span><p><b>"+aq.title+"</b></p>";var am={};var at=aq.uiText.split(" ");for(var al=0;al<at.length;al++){am[at[al].replace(/\W/g,"").toLowerCase()]={topicId:"",prob:0};}ao+="<p>";for(var al=0;al<aq.topicList.length;al++){var an=aq.topicList[al];ao+='<a class="region" id="top'+an+'">';ao+=aj(a.topics[an].nameTokens)+"</a>";topId="#top"+an;$("#docs").on({mouseenter:function(av){$(this).attr("class","current");$(".word").css({opacity:0});var au=".wtop"+(this.id).slice(3,(this.id).length);$(au).css({opacity:1});$(".wtop").css({opacity:0.3});},mouseleave:function(au){$(this).attr("class","region");$(".word").css({opacity:1});}},topId);for(var ap=0;ap<a.topics[an].wordsProb.length;ap++){word=a.topics[an].wordsProb[ap][0];prob=a.topics[an].wordsProb[ap][1];if((word in am)&&a.topics[an].score*prob>am[word].prob){am[word].topicId=an;am[word].prob=a.topics[an].score*prob;}}}ao+="</p><p>";for(var al=0;al<at.length;al++){ao+='<span class="word wtop';key=at[al].replace(/\W/g,"").toLowerCase();if(key in am){ao+=am[key].topicId;}ao+='">'+at[al]+" </span>";}ao+="</p></div></div>";return ao;}function ac(ar,an,aq){var ao=G("docs");ao.innerHTML=null;listLength=ar.length;for(var al=0;al<listLength&&al<5;al++){pmid=ar[al];doc=a.docs[pmid];titleLength=doc.title.length<21?doc.title.length:21;title=doc.title.slice(0,titleLength)+"...";ao.innerHTML+='<button class="region" id="but'+pmid+'">'+title+"</button>";ao.innerHTML+=T(pmid,doc);var ap="#but"+pmid;$("#docs").on("click",ap,function(at){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="block";});var am="#clo"+pmid;$("#docs").on("click",am,function(at){id="mod"+(this.id).slice(3,(this.id).length);document.getElementById(id).style.display="none";});}$("#docs").prepend("<br>");$("#docs").prepend("<span>Documents about the topic</span>");}function af(au,ao,az){var aq=new Array();var an=new Date(2050,1,1);var at=new Date(1960,1,1);for(var ay=0;ay<au.length;ay++){date=new Date(au[ay],1,1);if(date<an){an=date;}if(date>at){at=date;}aq.push(date);}var am=G(az);am.innerHTML=null;J=d3.select("#"+az).append("svg").attr("width",am.offsetWidth*2).attr("height",am.offsetHeight/3);var ap={top:10,right:30,bottom:30,left:40};var al=J.attr("width")-ap.left-ap.right;var aA=J.attr("height")-ap.top-ap.bottom;var ar=J.append("g").attr("transform","translate("+ap.left+","+ap.top+")");minEndDate=new Date(an.getFullYear()+5,1,1);at=minEndDate.getTime()<at.getTime()?at:minEndDate;var ax=d3.scaleTime().domain([an,at]).rangeRound([0,al]);var aB=d3.histogram().domain(ax.domain()).thresholds(ax.ticks(d3.timeYear))(aq);var aw=d3.scaleLinear().domain([0,ao]).range([aA,0]);var av=ar.selectAll(".bar").data(aB).enter().append("g").attr("class","bar").attr("transform",function(aC){return"translate("+ax(aC.x0)+","+aw(aC.length)+")";});av.append("rect").attr("x",1).attr("width",ax(aB[0].x1)-ax(aB[0].x0)-1).attr("height",function(aC){return aA-aw(aC.length);});if(aq.length>0){ar.append("g").attr("class","axisBlue").attr("transform","translate(0,"+aA+")").call(d3.axisBottom(ax));ar.append("g").attr("class","axisBlue").call(d3.axisLeft(aw));}title="";if(az=="tempHist"){title="Temporal trend for topic";}$("#"+az).prepend("<span>"+title+"</span>");}function ab(al){if(!d3.event.active){V.alphaTarget(0.3).restart();}al.fx=al.x;al.fy=al.y;}function Z(al){al.fx=d3.event.x;al.fy=d3.event.y;}function ad(al){if(!d3.event.active){V.alphaTarget(0);}al.fx=null;al.fy=null;}function X(al){if(al.uid>=0){al.color=ag(Math.floor(al.score/(ai/5)));}else{al.color=P(Math.floor(al.score/(ai/5)));}return al.color;}function R(al){if(al==0){return"";}else{if(al>59){return Math.floor(al/60)+" h. "+R(al%60);}else{return al+" avg score.";}}}function N(an){var ao=an.length;var am="";for(var al=0;al<ao;al++){am+=an[al]+" ";}return am;}},remove:function(){if(K!=null){K.transition().duration(1000).attr("r",0).remove();}if(J!=null){J.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove();}var N=G("docs");N.innerHTML=null;var N=G("tempHist");N.innerHTML=null;var N=G("forTempHist");N.innerHTML=null;x();}};}};var l=C.bubbleChart();jQuery("#cancerBtn").click(function(){H(this,"cancer");});jQuery("#diabetesBtn").click(function(){H(this,"diabetes");});jQuery("#ldaBtn").click(function(){t(this,"LDA");});jQuery("#lldaBtn").click(function(){t(this,"LLDA");});jQuery("#dualBtn").click(function(){t(this,"DUAL");});jQuery("#startDatepickerBtn").click(function(){B("#startdatepicker","#startDatepickerBtn",true);});jQuery("#endDatepickerBtn").click(function(){B("#enddatepicker","#endDatepickerBtn",false);});})();
