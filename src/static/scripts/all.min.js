...
(function(){var s=0,o=1,p=2,f=3,n=50;var q=s,B="diabetes",a;var m=0,A=0,e=null;window.onload=function(){$("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(757382400),maxDate:new Date(),onSelect:function(F){d(this,f);}});$("#datepicker").datepicker("setDate",new Date());b();u();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){C("topics").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(F){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(F){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function C(F){return document.getElementById(F);}function u(){var G=document.URL.split("/");var F=G[0]+"//"+G[2]+"/rpcNewSearch?keywords="+B;if(q==f){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;F+="&start_date="+startDate+"&end_date="+endDate;}F+="&limit="+n;var H=new XMLHttpRequest();H.open("GET",F,true);H.onreadystatechange=function(){if(H.readyState==4){h();if(H.status==200){g(H.responseText);}else{z();}}};H.send(null);k();b();}function g(F){try{a=JSON.parse(F);}catch(G){a=F;}if(a==""){C("topics").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(a.topics.length==0){C("topics").innerHTML="NO TOPIC FOUND!";return;}v();j();A=0;setTimeout(E,3000);}function z(F){C("topics").innerHTML=F||"YOU CRASHED IT!";}function b(){$("#topics").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function h(){$("#topics").empty();}function v(F){i.draw(F);}function r(F){i.remove(F);}function E(G,F){try{if(A){return;}if(!G){G=$("circle");}if(typeof F=="undefined"){F=0;}if(G[F]){e=$(G[F]).mouseover();setTimeout(function(){$(G[F]).mouseout();e=null;E(G,F+1);},5000);}}catch(H){}}function k(){if(e){e.mouseout();A=1;e=null;}}function j(F){var F="Topics extracted from medical abstracts published";if(q==s){F+=" within last 24 hours";}else{if(q==o){F+=" within last week";}else{if(q==p){F+=" within last month";}else{F+=" on "+jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}}}if(B=="diabetes"){F+=" about Diabetes";}else{F+=" about Cancer";}$("#topics").prepend("<span>"+F+"</span>");}function d(G,F){if(q!=F||F==f){q=F;r(u);l(G,F);}return false;}function D(G,F){if(B!=F){B=F;r(u);w(G);}return false;}function t(F){if(F){C("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}else{C("dateText").innerHTML="pick a date";}}function w(F){$("nav a").removeClass("current");$(F).addClass("current");}function l(G,F){if(F==f){t(true);x();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(G){m=true;x();t(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(G).addClass("active");}}}function x(){if(m){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}m=!m;return false;}function c(){if(q==s){return"ld";}else{if(q==o){return"lw";}else{if(q==p){return"lm";}}}}var y={bubbleChart:function(){var F=C("topics");return{draw:function(){F.innerHTML=null;var N=F.offsetWidth;var Y=F.offsetHeight;var Q=d3.select("#topics").append("svg").attr("width",N).attr("height",Y);var H=a.links;var K=d3.forceSimulation().force("link",d3.forceLink().id(function(aa){return aa.uid;})).force("charge",d3.forceManyBody()).force("center",d3.forceCenter(N/2,Y/2));var L=Q.append("g").attr("class","links").selectAll("line").data(H).enter().append("line").attr("stroke-width",function(aa){return Math.sqrt(aa.value);});var U=a.topics;var Z=U[0].score;var X=d3.scaleOrdinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]);var P=(3500/N)*(Z/50);var V=Q.selectAll(".node").data(U).enter().append("circle").attr("class","node").attr("cx",function(aa){return aa.x;}).attr("cy",function(aa){return aa.y;}).attr("r",0).style("fill",function(aa){return M(aa);}).style("stroke",function(ab,aa){return d3.rgb(ab.color).darker(2);}).call(d3.drag().on("start",R).on("drag",O).on("end",T));K.nodes(U).on("tick",J);K.force("link").links(H);function J(){L.attr("x1",function(aa){return aa.source.x;}).attr("y1",function(aa){return aa.source.y;}).attr("x2",function(aa){return aa.target.x;}).attr("y2",function(aa){return aa.target.y;});V.attr("cx",function(aa){return aa.x;}).attr("cy",function(aa){return aa.y;});}V.transition().duration(1000).attr("r",function(aa){return aa.score/P;});Q.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){k();var aa=d3.select(this);aa.moveToFront();var ab=this.__data__;return'<div class="tipsy-topic">'+G(ab.words)+'</div><span class="tipsy-time">'+I((ab.score*10).toFixed(3))+"</span>";}});$("circle").click(function(){var aa=this.__data__;S(aa.docs);W(aa.years);});function S(ac){var ab=C("docs");ab.innerHTML=null;listLength=ac.length;for(var aa=0;aa<listLength&&aa<6;aa++){ab.innerHTML+='<a href="'+ac[aa]+'"> Document #'+aa+"</a>";}}function W(ag){var ad=new Array();for(var ak=0;ak<ag.length;ak++){ad.push(new Date(ag[ak],1,1));}var aa=C("tempHist");aa.innerHTML=null;var ae=d3.select("#tempHist").append("svg").attr("width",aa.offsetWidth*2).attr("height",aa.offsetHeight/3);var ac={top:10,right:30,bottom:30,left:30};var ab=ae.attr("width")-ac.left-ac.right;var al=ae.attr("height")-ac.top-ac.bottom;var af=ae.append("g").attr("transform","translate("+ac.left+","+ac.top+")");var aj=d3.scaleTime().domain([new Date(2006,1,1),new Date(2017,1,1)]).rangeRound([0,ab]);var am=d3.histogram().domain(aj.domain()).thresholds(aj.ticks(d3.timeYear))(ad);var ai=d3.scaleLinear().domain([0,d3.max(am,function(an){return an.length;})]).range([al,0]);var ah=af.selectAll(".bar").data(am).enter().append("g").attr("class","bar").attr("transform",function(an){return"translate("+aj(an.x0)+","+ai(an.length)+")";});ah.append("rect").attr("x",1).attr("width",aj(am[0].x1)-aj(am[0].x0)-1).attr("height",function(an){return al-ai(an.length);});if(ad.length>0){af.append("g").attr("class","axisBlue").attr("transform","translate(0,"+al+")").call(d3.axisBottom(aj));af.append("g").attr("class","axisBlue").call(d3.axisLeft(ai));}}function R(aa){if(!d3.event.active){K.alphaTarget(0.3).restart();}aa.fx=aa.x;aa.fy=aa.y;}function O(aa){aa.fx=d3.event.x;aa.fy=d3.event.y;}function T(aa){if(!d3.event.active){K.alphaTarget(0);}aa.fx=null;aa.fy=null;}function M(aa){aa.color=X(Math.floor(aa.score/(Z/5)));return aa.color;}function I(aa){if(aa==0){return"";}else{if(aa>59){return Math.floor(aa/60)+" h. "+I(aa%60);}else{return aa+" avg prob.";}}}function G(ac){var ad=ac.length;var ab="";for(var aa=0;aa<ad;aa++){ab+=ac[aa]+" ";}return ab;}},remove:function(G){node.transition().duration(1000).attr("r",0).remove();svg.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",G);}};}};var i=y.bubbleChart();jQuery("#cancerBtn").click(function(){D(this,"cancer");});jQuery("#diabetesBtn").click(function(){D(this,"diabetes");});jQuery("#lastDayBtn").click(function(){d(this,s);});jQuery("#lastWeekBtn").click(function(){d(this,o);});jQuery("#lastMonthBtn").click(function(){d(this,p);});jQuery("#datepickerBtn").click(function(){x();});})();
